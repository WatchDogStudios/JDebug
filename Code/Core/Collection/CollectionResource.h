...
/// \brief Represents one resource to load / preload through an nsCollectionResource
struct NS_CORE_DLL nsCollectionEntry
{
  nsString m_sOptionalNiceLookupName; ///< Optional, can be used to lookup the resource at runtime with a nice name. E.g. "SkyTexture" instead of some GUID.
  nsString m_sResourceID;             ///< The ID / path to the resource to load.
  nsHashedString m_sAssetTypeName;
  nsUInt64 m_uiFileSize = 0;
};

/// \brief Describes a full nsCollectionResource, ie. lists all the resources that the collection contains
struct NS_CORE_DLL nsCollectionResourceDescriptor
{
  nsDynamicArray<nsCollectionEntry> m_Resources;

  void Save(nsStreamWriter& inout_stream) const;
  void Load(nsStreamReader& inout_stream);
};

using nsCollectionResourceHandle = nsTypedResourceHandle<class nsCollectionResource>;

/// \brief An nsCollectionResource is used to tell the engine about resources that it should preload in the background
///
...
/// In scenes this can be achieved using an nsCollectionComponent.
///
...
///
/// \note Once nsCollectionResource::PreloadResources() has been triggered, the resource will hold a handle to all referenced resources,
...
///       should get freed up.
class NS_CORE_DLL nsCollectionResource : public nsResource
{
  NS_ADD_DYNAMIC_REFLECTION(nsCollectionResource, nsResource);
  NS_RESOURCE_DECLARE_COMMON_CODE(nsCollectionResource);
  NS_RESOURCE_DECLARE_CREATEABLE(nsCollectionResource, nsCollectionResourceDescriptor);

public:
  nsCollectionResource();
  ~nsCollectionResource();

  /// \brief Registers the named resources in the collection with the nsResourceManager, such that they can be loaded by those names.
  ///
  ...
  /// \brief Removes the registered names from the nsResourceManager.
  ///
  /// Calling this twice has no effect.
  void UnregisterNames();

  /// \brief Puts up to the given number of resources for which a resource type could be found into the preload queue of the nsResourceManager.
  ///
  ...
  bool PreloadResources(nsUInt32 uiNumResourcesToPreload = nsMath::MaxValue<nsUInt32>());
  ...
  const nsCollectionResourceDescriptor& GetDescriptor() const;

  /// \brief Returns the current list of resources that have already been added to the preload list. See PreloadResources().
  nsArrayPtr<const nsTypelessResourceHandle> GetPreloadedResources() const { return m_PreloadedResources; }
...
private:
  ...
  mutable nsMutex m_PreloadMutex;
  bool m_bRegistered = false;
  nsCollectionResourceDescriptor m_Collection;
  nsDynamicArray<nsTypelessResourceHandle> m_PreloadedResources;
};
