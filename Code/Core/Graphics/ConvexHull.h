...
class NS_CORE_DLL nsConvexHullGenerator
{
public:
  struct Face
  {
    ...
    nsUInt16 m_uiVertexIdx[3];
  };

  nsConvexHullGenerator();
  ~nsConvexHullGenerator();
  ...
  void SetSimplificationMinTriangleAngle(nsAngle angle) { m_MinTriangleAngle = angle; }
  ...
  /// Default is 5 degree.
  void SetSimplificationFlatVertexNormalThreshold(nsAngle angle) { m_FlatVertexNormalThreshold = angle; }
  ...
  nsResult Build(const nsArrayPtr<const nsVec3> vertices);

  /// \brief When Build() was successful this can be called to retrieve the resulting vertices and triangles.
  void Retrieve(nsDynamicArray<nsVec3>& out_vertices, nsDynamicArray<Face>& out_faces);

  /// \brief Same as Retrieve() but only returns the vertices.
  void RetrieveVertices(nsDynamicArray<nsVec3>& out_vertices);

private:
  nsResult ComputeCenterAndScale(const nsArrayPtr<const nsVec3> vertices);
  nsResult StoreNormalizedVertices(const nsArrayPtr<const nsVec3> vertices);
  void StoreTriangle(nsUInt16 i, nsUInt16 j, nsUInt16 k);
  nsResult InitializeHull();
  nsResult ComputeHull();
  bool IsInside(nsUInt32 vtxId) const;
  void RemoveVisibleFaces(nsUInt32 vtxId);
  void PatchHole(nsUInt32 vtxId);
  ...
  nsResult ProcessVertices(const nsArrayPtr<const nsVec3> vertices);

  struct TwoSet
  {
    NS_ALWAYS_INLINE TwoSet()
    {
      a = 0xFFFF;
      b = 0xFFFF;
    }
    NS_ALWAYS_INLINE void Add(nsUInt16 x) { (a == 0xFFFF ? a : b) = x; }
    NS_ALWAYS_INLINE bool Contains(nsUInt16 x) { return a == x || b == x; }
    NS_ALWAYS_INLINE void Remove(nsUInt16 x) { (a == x ? a : b) = 0xFFFF; }
    NS_ALWAYS_INLINE int GetSize() { return (a != 0xFFFF) + (b != 0xFFFF); }
  ...
  nsAngle m_MinTriangleAngle = nsAngle::MakeFromDegree(22.0f);
  nsAngle m_FlatVertexNormalThreshold = nsAngle::MakeFromDegree(5);
  ...
  nsDynamicArray<nsVec3d> m_Vertices;
  ...
  nsDynamicArray<TwoSet> m_Edges;

  nsDeque<Triangle> m_Triangles;
};
