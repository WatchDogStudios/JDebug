...
nsInputDevice* nsInputDeviceMouseKeyboard::s_pMouseOver = nullptr;
...
void nsInputDeviceController::SetPhysicalControllerMapping(nsUInt8 uiPhysicalController, nsInt8 iVirtualController)
{
  NS_ASSERT_DEV(uiPhysicalController < MaxControllers, "Physical Controller Index {0} is larger than allowed ({1}).", uiPhysicalController, MaxControllers);
  NS_ASSERT_DEV(iVirtualController < MaxControllers, "Virtual Controller Index {0} is larger than allowed ({1}).", iVirtualController, MaxControllers);

  m_iPhysicalToVirtualControllerMapping[uiPhysicalController] = iVirtualController;

  if (iVirtualController < 0)
  {
    nsLog::Dev("Input from physical controller {} got deactivated", uiPhysicalController);
  }
  else
  {
    nsLog::Dev("Mapped physical controller {} to virtual controller {}", uiPhysicalController, iVirtualController);
  }
}

nsInt8 nsInputDeviceController::GetPhysicalControllerMapping(nsUInt8 uiPhysical) const
{
  NS_ASSERT_DEV(uiPhysical < MaxControllers, "Physical Controller Index {0} is larger than allowed ({1}).", uiPhysical, MaxControllers);

  return m_iPhysicalToVirtualControllerMapping[uiPhysical];
}

nsBitflags<nsPhysicalControllerInput> nsInputDeviceController::GetRecentPhysicalControllerInput(nsUInt8 uiPhysical) const
{
  NS_ASSERT_DEV(uiPhysical < MaxControllers, "Physical Controller Index {0} is larger than allowed ({1}).", uiPhysical, MaxControllers);
...
void nsInputDeviceController::AddVibrationTrack(nsUInt8 uiVirtual, Motor::Enum motor, float* pVibrationTrackValue, nsUInt32 uiSamples, float fScalingFactor)
{
...
void nsInputDeviceController::UpdateVibration(nsTime tTimeDifference)
{
  ...
  for (nsUInt8 c = 0; c < MaxControllers; ++c)
  {
    ...
    for (nsUInt8 p = 0; p < MaxControllers; ++p)
    {
      if (m_iPhysicalToVirtualControllerMapping[p] == c)
      {
        for (nsUInt32 m = 0; m < Motor::ENUM_COUNT; ++m)
        {
          fVibrationToApply[p][m] = nsMath::Max(m_fVibrationStrength[c][m], m_fVibrationTracks[c][m][m_uiVibrationTrackPos]);
        }
      }
    }
  }
