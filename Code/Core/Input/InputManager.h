...
public:
  ...
  {
    for (auto pDev = nsInputDevice::GetFirstInstance(); pDev; pDev = pDev->GetNextInstance())
    {
  ...
  /// \brief Fills the array with all nsInputDevice instances of the given base type.
  template <typename TYPE>
  static void GetInputDevicesOfType(nsDynamicArray<TYPE*>& out_devices)
  {
    out_devices.Clear();
  ...
  /// \brief Fills the array with all nsInputDevice instances of the given base type.
  static void GetInputDevicesOfType(const nsRTTI* pRtti, nsDynamicArray<nsInputDevice*>& out_devices);
