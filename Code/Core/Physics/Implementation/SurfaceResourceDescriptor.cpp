...
NS_BEGIN_STATIC_REFLECTED_TYPE(nsSurfaceInteraction, nsNoBase, 1, nsRTTIDefaultAllocator<nsSurfaceInteraction>)
{
  NS_BEGIN_PROPERTIES
  {
    ...
    NS_RESOURCE_MEMBER_PROPERTY("Prefab", m_hPrefab)->AddAttributes(new nsAssetBrowserAttribute("CompatibleAsset_Prefab", nsDependencyFlags::Package)),
...
NS_BEGIN_DYNAMIC_REFLECTED_TYPE(nsSurfaceResourceDescriptor, 2, nsRTTIDefaultAllocator<nsSurfaceResourceDescriptor>)
{
  NS_BEGIN_PROPERTIES
  {
    NS_RESOURCE_MEMBER_PROPERTY("BaseSurface", m_hBaseSurface)->AddAttributes(new nsAssetBrowserAttribute("CompatibleAsset_Surface")),// package+thumbnail so that it forbids circular dependencies
...
public:
  ...
  {
    NS_IGNORE_UNUSED(ref_context);
    NS_IGNORE_UNUSED(pGraph);
