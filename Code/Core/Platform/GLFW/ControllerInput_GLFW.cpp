...
public:
  ...
  virtual bool IsPhysicalControllerConnected(nsUInt8 uiPhysical) const override;
...
void nsControllerInputGlfw::UpdateInputSlotValues()
{
  ...
  for (nsUInt8 uiVirtual = 0; uiVirtual < MaxControllers; ++uiVirtual)
  {
    ...
    const nsInt8 iPhysical = GetPhysicalControllerMapping(uiVirtual);
...
bool nsControllerInputGlfw::IsPhysicalControllerConnected(nsUInt8 uiPhysical) const
{
  if (!m_bInitialized)
  {
    return false;
...
NS_STATICLINK_FILE(Core, Core_Platform_GLFW_ControllerInput_GLFW);
