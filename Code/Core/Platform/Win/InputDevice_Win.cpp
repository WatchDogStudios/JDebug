...
// clang-format off
NS_BEGIN_DYNAMIC_REFLECTED_TYPE(nsInputDeviceMouseKeyboard_Win, 1, nsRTTINoAllocator)
NS_END_DYNAMIC_REFLECTED_TYPE;
// clang-format on

#  define WM_USER_UPDATE_CURSOR (WM_USER + 1)

nsInputDeviceMouseKeyboard_Win* nsInputDeviceMouseKeyboard_Win::s_pGlobalInputHandler = nullptr;

nsInputDeviceMouseKeyboard_Win::nsInputDeviceMouseKeyboard_Win(nsMinWindows::HWND hWnd)
{
  m_hWnd = hWnd;
...
nsInputDeviceMouseKeyboard_Win::~nsInputDeviceMouseKeyboard_Win()
{
  if (!m_bShowCursor)
  {
    ShowCursor(true);
  ...
  if (s_pGlobalInputHandler == this)
  {
    ...
    nsLog::Dev("Global mouse/keyboard input handler destroyed.");
  }
}

void nsInputDeviceMouseKeyboard_Win::InitializeDevice()
{
  if (s_pGlobalInputHandler == this)
  {
  ...
  {
    nsLog::Dev("Window doesn't handle global mouse/keyboard input.");
  }
}

void nsInputDeviceMouseKeyboard_Win::RegisterInputSlots()
{
...
void nsInputDeviceMouseKeyboard_Win::ResetInputSlotValues()
{
...
void nsInputDeviceMouseKeyboard_Win::UpdateInputSlotValues()
{
...
void nsInputDeviceMouseKeyboard_Win::ApplyClipRect(nsMouseCursorClipMode::Enum mode)
{
  if (!m_bApplyClipRect)
    return;
  ...
  {
    RECT area;
    GetClientRect(nsMinWindows::ToNative(m_hWnd), &area);
    POINT p0, p1;
    p0.x = 0;
    p0.y = 0;
    p1.x = area.right;
    ...
    ClientToScreen(nsMinWindows::ToNative(m_hWnd), &p0);
    ClientToScreen(nsMinWindows::ToNative(m_hWnd), &p1);
...
void nsInputDeviceMouseKeyboard_Win::SetClipMouseCursor(nsMouseCursorClipMode::Enum mode)
{
...
void nsInputDeviceMouseKeyboard_Win::WindowMessage(nsMinWindows::UINT msg, nsMinWindows::WPARAM wparam, nsMinWindows::LPARAM lparam)
{
#  if NS_ENABLED(NS_MOUSEBUTTON_COMPATIBILTY_MODE)
  static nsInt32 s_iMouseCaptureCount = 0;
#  endif

  if (m_bFirstWndMsg && m_ClipCursorMode != nsMouseCursorClipMode::NoClip)
  {
    ...
    RECT r;
    GetWindowRect(nsMinWindows::ToNative(m_hWnd), &r);
    SetCursorPos(nsMath::Lerp(r.left, r.right, 0.5f), nsMath::Lerp(r.bottom, r.top, 0.5f));
  }
  ...
  switch (msg)
  {
    ...
    case WM_MOUSEMOVE:
    {
      ...
      if (s_pGlobalInputHandler == this)
      {
        ...
        m_InputSlotValues[nsInputSlot_MousePositionX] = m_vLocalMouseCoordinates.x;
        m_InputSlotValues[nsInputSlot_MousePositionY] = m_vLocalMouseCoordinates.y;
      }
...
#  if NS_ENABLED(NS_MOUSEBUTTON_COMPATIBILTY_MODE)
    ...
    case WM_LBUTTONDOWN:
      ...
      if (s_iMouseCaptureCount == 0)
        SetCapture(nsMinWindows::ToNative(m_hWnd));
...
void nsInputDeviceMouseKeyboard_Win::LocalizeButtonDisplayNames()
{
  NS_LOG_BLOCK("nsInputDeviceMouseKeyboard_Win::LocalizeButtonDisplayNames");
...
void nsInputDeviceMouseKeyboard_Win::SetShowMouseCursor(bool bShow)
{
  ...
  {
    PostMessageW(nsMinWindows::ToNative(m_hWnd), WM_USER_UPDATE_CURSOR, m_bShowCursor ? 1 : 0, 0);
  }
}

bool nsInputDeviceMouseKeyboard_Win::GetShowMouseCursor() const
{
  return m_bShowCursor;
}

void nsInputDeviceMouseKeyboard_Win::OnFocusLost()
{
  ...
  m_bApplyClipRect = true;
  ApplyClipRect(nsMouseCursorClipMode::NoClip);
...
NS_STATICLINK_FILE(Core, Core_Platform_Win_InputDevice_Win);
