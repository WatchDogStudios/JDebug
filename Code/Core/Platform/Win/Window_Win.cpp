...
#if NS_ENABLED(NS_PLATFORM_WINDOWS_DESKTOP) && NS_DISABLED(NS_SUPPORTS_GLFW)
...
{
  nsWindowWin* pWindow = reinterpret_cast<nsWindowWin*>(GetWindowLongPtrW(hWnd, GWLP_USERDATA));

  if (pWindow != nullptr && pWindow->IsInitialized())
  {
    if (auto pInput = nsDynamicCast<nsInputDeviceMouseKeyboard_Win*>(pWindow->GetInputDevice()))
    {
...
nsWindowWin::~nsWindowWin()
{
  DestroyWindow();
}

nsResult nsWindowWin::InitializeWindow()
{
  ...
  if (!RegisterClassExW(&windowClass)) /// \todo test & support for multiple windows
  {
    ...
    {
      nsLog::Error("Failed to create nsWindow window class! (error code '{0}')", nsArgErrorCode(error));
      return NS_FAILURE;
    }
  }
  ...
  if (m_CreationDescription.m_bSetForegroundOnInit && !nsSystemInformation::IsDebuggerAttached())
  {
  ...
  if (m_CreationDescription.m_WindowMode == nsWindowMode::WindowFixedResolution &&
      (m_CreationDescription.m_Resolution.width != nsUInt32(r.right - r.left) ||
        m_CreationDescription.m_Resolution.height != nsUInt32(r.bottom - r.top)))
  {
  ...
  auto pInput = NS_DEFAULT_NEW(nsInputDeviceMouseKeyboard_Win, nsMinWindows::FromNative(windowHandle));
...
void nsWindowWin::DestroyWindow()
{
  if (!m_bInitialized)
    return;

  if (auto pInput = nsDynamicCast<nsInputDeviceMouseKeyboard_Win*>(GetInputDevice()))
  {
    pInput->SetClipMouseCursor(nsMouseCursorClipMode::NoClip);
  }
...
nsResult nsWindowWin::Resize(const nsSizeU32& newWindowSize)
{
  auto windowHandle = nsMinWindows::ToNative(m_hWindowHandle);
...
void nsWindowWin::ProcessWindowMessages()
{
  if (!m_bInitialized)
    return;
...
void nsWindowWin::OnResize(const nsSizeU32& newWindowSize)
{
  nsLog::Info("Window resized to ({0}, {1})", newWindowSize.width, newWindowSize.height);
}

nsWindowHandle nsWindowWin::GetNativeWindowHandle() const
{
