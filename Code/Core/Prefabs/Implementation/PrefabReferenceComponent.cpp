...
NS_BEGIN_COMPONENT_TYPE(nsPrefabReferenceComponent, 4, nsComponentMode::Static)
{
  NS_BEGIN_PROPERTIES
  {
    NS_RESOURCE_ACCESSOR_PROPERTY("Prefab", GetPrefab, SetPrefab)->AddAttributes(new nsAssetBrowserAttribute("CompatibleAsset_Prefab")),
    NS_ACCESSOR_PROPERTY("ShowShapeIcons", GetShowShapeIcons, SetShowShapeIcons),
...
enum class PrefabComponentFlags : nsUInt8
{
...
void nsPrefabReferenceComponent::SetShowShapeIcons(bool bShow)
{
  SetUserFlag((nsUInt8)PrefabComponentFlags::ShowShapeIcons, bShow);
...
bool nsPrefabReferenceComponent::GetShowShapeIcons() const
{
  return GetUserFlag((nsUInt8)PrefabComponentFlags::ShowShapeIcons);
}
...
void nsPrefabReferenceComponent::InstantiatePrefab()
{
  ...
  if (m_hPrefab.IsValid())
  {
    ...
    if (GetUniqueID() != nsInvalidIndex)
    {
      ...
      auto FixComponent = [](nsGameObject* pChild, nsUInt32 uiUniqueID, bool bShowShapeIcons)
      {
...
{
  if (GetUserFlag((nsUInt8)PrefabComponentFlags::SelfDeletion))
  {
...
void nsPrefabReferenceComponent::OnSimulationStarted()
{
  ...
  if (GetUniqueID() == nsInvalidIndex)
  {
    SetUserFlag((nsUInt8)PrefabComponentFlags::SelfDeletion, true);
...
void nsPrefabReferenceComponentManager::Update(const nsWorldModule::UpdateContext& context)
{
  ...
  for (auto hComp : m_ComponentsToUpdate)
  {
    ...
    pComponent->SetUserFlag((nsUInt8)PrefabComponentFlags::InUpdateList, false);
...
{
  if (!pComponent->GetUserFlag((nsUInt8)PrefabComponentFlags::InUpdateList))
  {
    m_ComponentsToUpdate.PushBack(pComponent->GetHandle());
    pComponent->SetUserFlag((nsUInt8)PrefabComponentFlags::InUpdateList, true);
  }
}
