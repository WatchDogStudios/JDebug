...
nsResource::DoUpdate nsResource::UpdateGraphicsResource = nsResource::DoUpdate::OnAnyThread;
...
{
  NS_LOG_BLOCK("nsResource::UnloadData", GetResourceID());
...
void nsResource::CallUpdateContent(nsStreamReader* Stream)
{
  ...
  NS_LOG_BLOCK("nsResource::UpdateContent", GetResourceDescription());
...
void nsResource::VerifyAfterCreateResource(const nsResourceLoadDesc& ld)
{
  ...
  nsLog::Debug("Created {0} - '{1}' ", GetDynamicRTTI()->GetTypeName(), nsArgSensitive(GetResourceIdOrDescription(), "ResourceDesc"));
}
