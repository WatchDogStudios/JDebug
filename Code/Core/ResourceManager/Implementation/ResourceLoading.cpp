...
void nsResourceManager::InternalPreloadResource(nsResource* pResource, bool bHighestPriority)
{
  ...
  if (pResource->GetBaseResourceFlags().IsSet(nsResourceFlags::IsCreatedResource) && !pResource->GetBaseResourceFlags().IsSet(nsResourceFlags::HasCustomDataLoader))
    return;
...
void nsResourceManager::RunWorkerTask()
{
  if (s_pState->m_bShutdown)
    return;
...
bool nsResourceManager::ReloadResource(nsResource* pResource, bool bForce)
{
  ...
  {
    s_pState->m_ResourcesToUnloadOnMainThread.Insert(nsTempHashedString(pResource->GetResourceID()), pResource->GetDynamicRTTI());
  }
...
void nsResourceManager::EnsureResourceLoadingState(nsResource* pResource, const nsResourceState RequestedState)
{
  return EnsureResourceCondition(pResource, [=]() -> bool
    { return (nsInt32)pResource->GetLoadingState() >= (nsInt32)RequestedState ||
             (pResource->GetLoadingState() == nsResourceState::LoadedResourceMissing); });
}

void nsResourceManager::EnsureResourceCondition(nsResource* pResourceToLoad, const nsDelegate<bool()>& condition)
{
  ...
  while (!condition())
  {
    ...
    else
    {
      ...
      nsTaskSystem::WaitForCondition(condition);
    }
  }
}
