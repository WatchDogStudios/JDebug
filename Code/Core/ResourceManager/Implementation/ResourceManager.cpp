...
void nsResourceManager::PerFrameUpdate()
{
  ...
  s_pState->m_LastFrameUpdate = nsClock::GetGlobalClock()->GetLastUpdateTime();
...
nsResource* nsResourceManager::GetResource(const nsRTTI* pRtti, nsStringView sResourceID, bool bIsReloadable)
{
  ...
  NS_ASSERT_DEBUG(pRtti != nullptr, "There is no RTTI information available for the given resource type '{0}'", NS_PP_STRINGIFY(ResourceType));
  ...
  NS_ASSERT_DEBUG(pRtti->GetAllocator() != nullptr && pRtti->GetAllocator()->CanAllocate(), "There is no RTTI allocator available for the given resource type '{0}'", NS_PP_STRINGIFY(ResourceType));
  ...
  nsResource*& pResource = lr.m_Resources[sHashedResourceID];
  if (pResource != nullptr)
    return pResource;

  pResource = pRtti->GetAllocator()->Allocate<nsResource>();
  pResource->m_Priority = s_pState->m_ResourceTypePriorities.GetValueOrDefault(pRtti, nsResourcePriority::Medium);
  pResource->SetUniqueID(sResourceID, bIsReloadable);
  pResource->m_Flags.AddOrRemove(nsResourceFlags::ResourceHasTypeFallback, pResource->HasResourceTypeLoadingFallback());
