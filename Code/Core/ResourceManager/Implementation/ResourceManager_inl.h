...
NS_FORCE_INLINE ResourceType* nsResourceManager::GetResource(nsStringView sResourceID, bool bIsReloadable)
{
  return static_cast<ResourceType*>(GetResource(nsGetStaticRTTI<ResourceType>(), sResourceID, bIsReloadable));
}

template <typename ResourceType>
NS_FORCE_INLINE nsTypedResourceHandle<ResourceType> nsResourceManager::LoadResource(nsStringView sResourceID)
{
...
template <typename ResourceType, typename DescriptorType>
nsTypedResourceHandle<ResourceType> nsResourceManager::CreateResourceInternal(nsStringView sResourceID, DescriptorType&& descriptor, nsStringView sResourceDescription, bool bAllowGetFallback)
{
  ...
  NS_LOG_BLOCK("nsResourceManager::CreateResource", sResourceID);
  {
    ...
    {
      hResource = nsTypedResourceHandle<ResourceType>(GetResource<ResourceType>(sResourceID, false));
      pResource = BeginAcquireResource(hResource, nsResourceAcquireMode::PointerOnly);
  ...
  if (!pResource)
  {
    ...
    pResource = BeginAcquireResource(hResource, nsResourceAcquireMode::BlockTillLoaded);
      ...
      if (IsQueuedForLoading(pResource) == false && pResource->GetNumQualityLevelsLoadable() > 0)
      {
        ...
        if (GetForceNoFallbackAcquisition() > 0)
        {
          EnsureResourceCondition(pResource, [=]() -> bool
            { return ((nsInt32)pResource->GetLoadingState() >= (nsInt32)nsResourceState::Loaded && pResource->GetNumQualityLevelsLoadable() == 0) ||
                     (pResource->GetLoadingState() == nsResourceState::LoadedResourceMissing); });
        }
      }
    }
  }
