...
NS_ALWAYS_INLINE void MigrateResourceRefCount(nsResource* pResource, const void* pOldOwner, const void* pNewOwner)
{
  ...
  NS_IGNORE_UNUSED(pOldOwner);
  NS_IGNORE_UNUSED(pNewOwner);
}
#endif
...
public:
  ...
  nsStringView GetResourceID() const;
  ...
  /// If the handle is not valid, an empty string is returned.
  nsStringView GetResourceIdOrDescription() const;
...
public:
  ...
  /// \brief Returns the Resource ID of the exact resource that this handle points to, without acquiring the resource.
  /// The handle must be valid.
  NS_ALWAYS_INLINE nsStringView GetResourceID() const { return m_hTypeless.GetResourceID(); }

  /// \brief The returns the resource description, if available, otherwise the resource ID.
  /// This is mainly for logging, where you want the more user friendly description, but the ID, if no description is available.
  /// If the handle is not valid, an empty string is returned.
  NS_ALWAYS_INLINE nsStringView GetResourceIdOrDescription() const { return m_hTypeless.GetResourceIdOrDescription(); }
