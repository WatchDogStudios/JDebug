...
// clang-format off
NS_BEGIN_COMPONENT_TYPE(nsScriptComponent, 2, nsComponentMode::Static)
{
  NS_BEGIN_PROPERTIES
  {
    ...
    NS_ACCESSOR_PROPERTY("UpdateOnlyWhenSimulating", GetUpdateOnlyWhenSimulating, SetUpdateOnlyWhenSimulating)->AddAttributes(new nsDefaultValueAttribute(true)),
    NS_RESOURCE_ACCESSOR_PROPERTY("ScriptClass", GetScriptClass, SetScriptClass)->AddAttributes(new nsAssetBrowserAttribute("CompatibleAsset_ScriptClass", nsDependencyFlags::Package)),
...
void nsScriptComponent::SetUpdateOnlyWhenSimulating(bool bUpdate)
{
...
void nsScriptComponent::BroadcastEventMsg(nsEventMessage& ref_msg)
{
  const nsRTTI* pType = ref_msg.GetDynamicRTTI();
...
void nsScriptComponent::InstantiateScript(bool bActivate)
{
  ...
  {
    nsLog::Error("Failed to load script '{}'", GetScriptClass().GetResourceIdOrDescription());
    return;
  }
