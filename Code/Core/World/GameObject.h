...
struct nsTransformPreservation
{
...
NS_DECLARE_REFLECTABLE_TYPE(NS_CORE_DLL, nsTransformPreservation);
...
public:
  ...
  /// \brief Adds nsObjectFlags::HideShapeIcon to the object. See the flag for details.
  void SetHideShapeIcon() { m_Flags.Add(nsObjectFlags::HideShapeIcon); }

  /// \brief Checks whether the nsObjectFlags::HideShapeIcon flag is set on this object.
  bool IsShapeIconHidden() const { return m_Flags.IsSet(nsObjectFlags::HideShapeIcon); }
  ...
  const nsHashedString& GetNameHashed() const;
  ...
  void SetParent(const nsGameObjectHandle& hParent, nsTransformPreservation::Enum preserve = nsTransformPreservation::PreserveGlobal);
  ...
  void AddChild(const nsGameObjectHandle& hChild, nsTransformPreservation::Enum preserve = nsTransformPreservation::PreserveGlobal);

  /// \brief Adds the given objects as child objects.
  void AddChildren(const nsArrayPtr<const nsGameObjectHandle>& children, nsTransformPreservation::Enum preserve = nsTransformPreservation::PreserveGlobal);

  /// \brief Detaches the given child object from this object and makes it a top-level object.
  void DetachChild(const nsGameObjectHandle& hChild, nsTransformPreservation::Enum preserve = nsTransformPreservation::PreserveGlobal);

  /// \brief Detaches the given child objects from this object and makes them top-level objects.
  void DetachChildren(const nsArrayPtr<const nsGameObjectHandle>& children, nsTransformPreservation::Enum preserve = nsTransformPreservation::PreserveGlobal);
  ...
  nsGameObject* FindChildByName(const nsTempHashedString& sName, bool bRecursive = true); // [tested]

  /// \brief Searches for a child object with the given name. Optionally traverses the entire hierarchy.
  const nsGameObject* FindChildByName(const nsTempHashedString& sName, bool bRecursive = true) const; // [tested]
  ...
  nsGameObject* FindChildByPath(nsStringView sPath); // [tested]

  /// \brief Const overload of FindChildByPath()
  const nsGameObject* FindChildByPath(nsStringView sPath) const; // [tested]
  ...
  nsGameObject* SearchForChildByNameSequence(nsStringView sObjectSequence, const nsRTTI* pExpectedComponent = nullptr); // [tested]

  /// \brief Const overload of SearchForChildByNameSequence()
  const nsGameObject* SearchForChildByNameSequence(nsStringView sObjectSequence, const nsRTTI* pExpectedComponent = nullptr) const; // [tested]
  ...
  void SetGlobalRotationToLookAt(const nsVec3& vTargetPosition, const nsVec3& vUp = nsVec3::MakeAxisZ());
  ...
  void SetGlobalTransformToLookAt(const nsVec3& vOwnPosition, const nsVec3& vTargetPosition, const nsVec3& vUp = nsVec3::MakeAxisZ());
  ...
  /// \brief Tries to find a component of the given base type in the objects components list and returns the first match.
  [[nodiscard]] bool TryGetComponentOfBaseType(const nsRTTI* pType, nsComponent*& out_pComponent);

  /// \brief Tries to find a component of the given base type in the objects components list and returns the first match.
  [[nodiscard]] bool TryGetComponentOfBaseType(const nsRTTI* pType, const nsComponent*& out_pComponent) const;
  ...
  bool HasTag(const nsTempHashedString& sTagName) const;
  ...
  nsVisibilityState::Enum GetVisibilityState(nsUInt32 uiNumFramesBeforeInvisible = 5) const;
...
private:
  ...
  nsGameObject* Reflection_FindChildByName(const nsTempHashedString& sName, bool bRecursive) { return FindChildByName(sName, bRecursive); }
  nsGameObject* Reflection_FindChildByPath(nsStringView sPath) { return FindChildByPath(sPath); }
