...
// clang-format off
NS_BEGIN_STATIC_REFLECTED_ENUM(nsTransformPreservation, 1)
  NS_ENUM_CONSTANTS(nsTransformPreservation::PreserveLocal, nsTransformPreservation::PreserveGlobal)
...
NS_BEGIN_STATIC_REFLECTED_TYPE(nsGameObject, nsNoBase, 1, nsRTTINoAllocator)
{
  NS_BEGIN_PROPERTIES
  {
    ...
    NS_SET_ACCESSOR_PROPERTY("Tags", GetTags, Reflection_SetTag, Reflection_RemoveTag)->AddAttributes(new nsTagSetWidgetAttribute("Default"), new nsDefaultValueAttribute(GetDefaultTags())),
  ...
  NS_BEGIN_FUNCTIONS
  {
    ...
    NS_SCRIPT_FUNCTION_PROPERTY(SetHideShapeIcon),
    NS_SCRIPT_FUNCTION_PROPERTY(IsShapeIconHidden),

    NS_SCRIPT_FUNCTION_PROPERTY(HasName, In, "Name"),
    NS_SCRIPT_FUNCTION_PROPERTY(HasTag, In, "TagName"),

    NS_SCRIPT_FUNCTION_PROPERTY(Reflection_GetParent),
    NS_SCRIPT_FUNCTION_PROPERTY(Reflection_FindChildByName, In, "Name", In, "Recursive")->AddFlags(nsPropertyFlags::PureFunction),
    NS_SCRIPT_FUNCTION_PROPERTY(Reflection_FindChildByPath, In, "Path")->AddFlags(nsPropertyFlags::PureFunction),
    ...
    NS_SCRIPT_FUNCTION_PROPERTY(SetGlobalRotationToLookAt, In, "TargetPosition", In, "Up")->AddAttributes(new nsFunctionArgumentAttributes(1, new nsDefaultValueAttribute(nsVec3::MakeAxisZ()))),
    NS_SCRIPT_FUNCTION_PROPERTY(SetGlobalTransformToLookAt, In, "OwnPosition", In, "TargetPosition", In, "Up")->AddAttributes(new nsFunctionArgumentAttributes(2, new nsDefaultValueAttribute(nsVec3::MakeAxisZ()))),
    ...
    NS_SCRIPT_FUNCTION_PROPERTY(GetStableRandomSeed),
  }
  NS_END_FUNCTIONS;
...
void nsGameObject::Reflection_SetTag(const char* szTagName)
{
  if (nsStringUtils::IsNullOrEmpty(szTagName))
    return;

  const nsTag& tag = nsTagRegistry::GetGlobalRegistry().RegisterTag(szTagName);
  SetTag(tag);
}

void nsGameObject::Reflection_RemoveTag(const char* szTagName)
{
  if (nsStringUtils::IsNullOrEmpty(szTagName))
    return;

  if (const nsTag* pTag = nsTagRegistry::GetGlobalRegistry().GetTagByName(nsTempHashedString(szTagName)))
  {
    RemoveTag(*pTag);
  }
}
...
void nsGameObject::Reflection_AddChild(nsGameObject* pChild)
{
  ...
  AddChild(pChild->GetHandle(), nsTransformPreservation::PreserveLocal);
...
{
  DetachChild(pChild->GetHandle(), nsTransformPreservation::PreserveLocal);
...
void nsGameObject::SetParent(const nsGameObjectHandle& hParent, nsTransformPreservation::Enum preserve)
{
...
void nsGameObject::AddChild(const nsGameObjectHandle& hChild, nsTransformPreservation::Enum preserve)
{
  nsWorld* pWorld = GetWorld();
...
void nsGameObject::DetachChild(const nsGameObjectHandle& hChild, nsTransformPreservation::Enum preserve)
{
  nsWorld* pWorld = GetWorld();
...
const nsGameObject* nsGameObject::FindChildByName(const nsTempHashedString& sName, bool bRecursive /*= true*/) const
{
  nsGameObject* pThis = const_cast<nsGameObject*>(this);
...
const nsGameObject* nsGameObject::FindChildByPath(nsStringView sPath) const
{
  nsGameObject* pThis = const_cast<nsGameObject*>(this);
...
nsGameObject* nsGameObject::SearchForChildByNameSequence(nsStringView sObjectSequence, const nsRTTI* pExpectedComponent /*= nullptr*/)
{
  if (sObjectSequence.IsEmpty())
  {
    ...
    {
      const nsComponent* pComp = nullptr;
...
const nsGameObject* nsGameObject::SearchForChildByNameSequence(nsStringView sObjectSequence, const nsRTTI* pExpectedComponent /*= nullptr*/) const
{
  nsGameObject* pThis = const_cast<nsGameObject*>(this);
...
void nsGameObject::SetGlobalRotationToLookAt(const nsVec3& vTargetPosition, const nsVec3& vUp /*= nsVec3::MakeAxisZ()*/)
{
  const nsVec3 vDir = vTargetPosition - GetGlobalPosition();
  NS_ASSERT_DEV(!vDir.IsZero(0.0001f), "Own position and target position must differ.");

  const nsVec3 vFwd = vDir.GetNormalized();
  const nsVec3 vRight = vUp.CrossRH(vFwd).GetNormalized();
  const nsVec3 vUp2 = vFwd.CrossRH(vRight).GetNormalized();
  ...
  SetGlobalRotation(nsQuat::MakeFromMat3(mLook));
}

void nsGameObject::SetGlobalTransformToLookAt(const nsVec3& vOwnPosition, const nsVec3& vTargetPosition, const nsVec3& vUp /*= nsVec3::MakeAxisZ()*/)
{
  const nsVec3 vDir = vTargetPosition - vOwnPosition;
  NS_ASSERT_DEV(!vDir.IsZero(0.0001f), "Own position and target position must differ.");
  ...
  SetGlobalTransform(nsTransform(vOwnPosition, nsQuat::MakeFromMat3(mLook)));
}
...
nsVisibilityState::Enum nsGameObject::GetVisibilityState(nsUInt32 uiNumFramesBeforeInvisible) const
{
  if (!m_pTransformationData->m_hSpatialData.IsInvalidated())
  {
