...
NS_ALWAYS_INLINE const nsHashedString& nsGameObject::GetNameHashed() const
{
  return m_sName;
}
...
NS_ALWAYS_INLINE void nsGameObject::AddChildren(const nsArrayPtr<const nsGameObjectHandle>& children, nsTransformPreservation::Enum preserve)
{
...
NS_ALWAYS_INLINE void nsGameObject::DetachChildren(const nsArrayPtr<const nsGameObjectHandle>& children, nsTransformPreservation::Enum preserve)
{
  for (nsUInt32 i = 0; i < children.GetCount(); ++i)
  {
...
NS_ALWAYS_INLINE bool nsGameObject::HasTag(const nsTempHashedString& sTagName) const
{
