...
struct nsSpatialSystem_RegularGrid::Grid
{
  ...
  NS_FORCE_INLINE void ForEachCellInBox(const nsSimdBBox& box, Functor func) const
  {
    ...
    const nsUInt64 uiHashGridCost = nsUInt64(iNumIterations) * 10;
    if (uiHashGridCost > m_Cells.GetCount())
    {
      for (auto& pCell : m_Cells)
      {
        ...
        if (func(*pCell) == nsVisitorExecution::Stop)
          return;
      }
    }
    else
...
namespace nsInternal
{
  struct QueryHelper
  {
    ...
    static nsVisitorExecution::Enum ShapeQueryCallback(const nsSpatialSystem_RegularGrid::Cell& cell, const nsSpatialSystem::QueryParams& queryParams, nsSpatialSystem_RegularGrid::Stats& ref_stats, void* pUserData, nsVisibilityState::Enum visType)
    {
      NS_IGNORE_UNUSED(visType);
    ...
    static nsVisitorExecution::Enum FrustumQueryCallback(const nsSpatialSystem_RegularGrid::Cell& cell, const nsSpatialSystem::QueryParams& queryParams, nsSpatialSystem_RegularGrid::Stats& ref_stats, void* pUserData, nsVisibilityState::Enum visType)
    {
...
void nsSpatialSystem_RegularGrid::FindVisibleObjects(const nsFrustum& frustum, const QueryParams& queryParams, nsDynamicArray<const nsGameObject*>& out_Objects, nsSpatialSystem::IsOccludedFunc IsOccluded, nsVisibilityState::Enum visType) const
{
  NS_PROFILE_SCOPE("nsSpatialSystem_RegularGrid::FindVisibleObjects");
...
nsVisibilityState::Enum nsSpatialSystem_RegularGrid::GetVisibilityState(const nsSpatialDataHandle& hData, nsUInt32 uiNumFramesBeforeInvisible) const
{
  Data* pData = nullptr;
  NS_VERIFY(m_DataTable.TryGetValue(hData.GetInternalID(), pData), "Invalid spatial data handle");
  ...
  return static_cast<nsVisibilityState::Enum>(uiLastVisibilityType);
}
...
void nsSpatialSystem_RegularGrid::ForEachCellInBoxInMatchingGrids(const nsSimdBBox& box, const QueryParams& queryParams, CellCallback noFilterCallback, CellCallback filterByTagsCallback, void* pUserData, nsVisibilityState::Enum visType) const
{
#if NS_ENABLED(NS_COMPILE_FOR_DEVELOPMENT)
  if (queryParams.m_pStats != nullptr)
  {
