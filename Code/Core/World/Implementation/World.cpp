...
{
  NS_IGNORE_UNUSED(hThis);
  NS_IGNORE_UNUSED(sProperty);
...
NS_BEGIN_STATIC_REFLECTED_TYPE(nsWorld, nsNoBase, 1, nsRTTINoAllocator)
{
  NS_BEGIN_FUNCTIONS
  {
    NS_SCRIPT_FUNCTION_PROPERTY(DeleteObjectDelayed, In, "GameObject", In, "DeleteEmptyParents")->AddAttributes(new nsFunctionArgumentAttributes(1, new nsDefaultValueAttribute(true))),
    NS_SCRIPT_FUNCTION_PROPERTY(Reflection_TryGetObjectWithGlobalKey, In, "GlobalKey")->AddFlags(nsPropertyFlags::PureFunction),
    NS_SCRIPT_FUNCTION_PROPERTY(Reflection_SearchForObject, In, "SearchPath", In, "ReferenceObject")->AddFlags(nsPropertyFlags::PureFunction),
    NS_SCRIPT_FUNCTION_PROPERTY(Reflection_GetClock)->AddFlags(nsPropertyFlags::PureFunction),
    NS_SCRIPT_FUNCTION_PROPERTY(Reflection_GetRandomNumberGenerator)->AddFlags(nsPropertyFlags::PureFunction),
  }
  NS_END_FUNCTIONS;
}
    ...
    if (pInitBatch->m_ComponentsToInitialize.IsEmpty())
    {
      ...
      else
      {
        *pCompletionFactor = 1.0;
        ...
        NS_ASSERT_DEV(m_Data.m_pDefaultInitBatch != pInitBatch, "");
...
void nsWorld::Update()
{
  ...
  // pre-async phase
  {
    ...
    UpdateSynchronous(m_Data.m_UpdateFunctions[nsWorldUpdatePhase::PreAsync]);
  }
  ...
  // post-async phase
  {
    ...
    UpdateSynchronous(m_Data.m_UpdateFunctions[nsWorldUpdatePhase::PostAsync]);
  }
  ...
  // post-transform phase
  {
    ...
    UpdateSynchronous(m_Data.m_UpdateFunctions[nsWorldUpdatePhase::PostTransform]);
  }
...
nsRandom* nsWorld::Reflection_GetRandomNumberGenerator()
{
  return &m_Data.m_Random;
}

void nsWorld::SetParent(nsGameObject* pObject, nsGameObject* pNewParent, nsTransformPreservation::Enum preserve)
{
...
void nsWorld::SetObjectGlobalKey(nsGameObject* pObject, const nsHashedString& sGlobalKey)
{
  if (auto it = m_Data.m_GlobalKeyToIdTable.Find(sGlobalKey.GetHash()); it.IsValid())
  {
    ...
    nsLog::Warning("An object with the global key '{}' already exists. Overwriting with different object reference.", sGlobalKey);
#endif

    NS_VERIFY(m_Data.m_IdToGlobalKeyTable.Remove(it.Value().m_InstanceIndex), "Implementation error.");
...
void nsWorld::RegisterUpdateFunction(const nsComponentManagerBase::UpdateFunctionDesc& desc)
{
  ...
  NS_ASSERT_DEV(desc.m_Phase != nsWorldUpdatePhase::Async || desc.m_DependsOn.GetCount() == 0, "Asynchronous update functions must not have dependencies");
...
void nsWorld::UpdateAsynchronous()
{
  ...
  nsDynamicArrayBase<nsInternal::WorldData::RegisteredUpdateFunction>& updateFunctions = m_Data.m_UpdateFunctions[nsWorldUpdatePhase::Async];
  ...
  for (auto& updateFunction : updateFunctions)
  {
    ...
    const nsUInt32 uiGranularity = (updateFunction.m_uiAsyncPhaseBatchSize != 0) ? updateFunction.m_uiAsyncPhaseBatchSize : uiTotalCount;
...
void nsWorld::ProcessUpdateFunctionsToDeregister()
{
  CheckForWriteAccess();

  for (const nsWorldModule::UpdateFunctionDesc& updateFunction : m_Data.m_UpdateFunctionsToDeregister)
  {
...
void nsWorld::DeregisterUpdateFunctionInternal(const nsWorldModule::UpdateFunctionDesc& desc)
{
  nsDynamicArrayBase<nsInternal::WorldData::RegisteredUpdateFunction>& updateFunctions = m_Data.m_UpdateFunctions[desc.m_Phase.GetValue()];
...
void nsWorld::DeregisterUpdateFunctionsInternal(nsWorldModule* pModule)
{
  CheckForWriteAccess();

  for (nsUInt32 phase = nsWorldUpdatePhase::PreAsync; phase < nsWorldUpdatePhase::COUNT; ++phase)
  {
...
void nsWorld::PatchHierarchyData(nsGameObject* pObject, nsTransformPreservation::Enum preserve)
{
  nsGameObject* pParent = pObject->GetParent();
  ...
  if (preserve == nsTransformPreservation::Enum::PreserveGlobal)
  {
...
nsGameObject* nsWorld::SearchForObject(nsStringView sSearchPath, nsGameObject* pRefObj, const nsRTTI* pExpectedComponent)
{
  // Possible paths:
  //
  // rel/path
  ...
  {
    nsStringView sGlobalKey;

    if (const char* szSep = sSearchPath.FindSubString("/"))
    {
      sGlobalKey = nsStringView(sSearchPath.GetStartPointer(), szSep);
    ...
    if (!TryGetObjectWithGlobalKey(nsTempHashedString(sGlobalKey), pRefObj))
    {
      return nullptr;
    }
  }
  ...
  {
    nsStringView sParentName;

    if (const char* szSep = sSearchPath.FindSubString("/"))
    {
      sParentName = nsStringView(sSearchPath.GetStartPointer(), szSep);
      sSearchPath.SetStartPosition(szSep + 1);
    }
    else
    {
    ...
    const nsTempHashedString sStartName(sParentName);
...
const nsGameObject* nsWorld::SearchForObject(nsStringView sSearchPath, const nsGameObject* pReferenceObject /*= nullptr*/, const nsRTTI* pExpectedComponent /*= nullptr*/) const
{
  nsWorld* pThis = const_cast<nsWorld*>(this);
  nsGameObject* pRef = const_cast<nsGameObject*>(pReferenceObject);
