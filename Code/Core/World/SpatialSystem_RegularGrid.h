...
private:
  ...
  void FindVisibleObjects(const nsFrustum& frustum, const QueryParams& queryParams, nsDynamicArray<const nsGameObject*>& out_Objects, nsSpatialSystem::IsOccludedFunc IsOccluded, nsVisibilityState::Enum visType) const override;

  nsVisibilityState::Enum GetVisibilityState(const nsSpatialDataHandle& hData, nsUInt32 uiNumFramesBeforeInvisible) const override;
  ...
  struct Data
  {
    ...
    nsUInt64 m_uiGridBitmask : MAX_NUM_GRIDS; ///< Bitmask indicating which grids contain this object
    nsUInt64 m_uiAlwaysVisible : 1;           ///< Whether this object is always visible (bypasses spatial queries)
  };
  ...
  struct Stats;
  using CellCallback = nsDelegate<nsVisitorExecution::Enum(const Cell&, const QueryParams&, Stats&, void*, nsVisibilityState::Enum)>;
  void ForEachCellInBoxInMatchingGrids(const nsSimdBBox& box, const QueryParams& queryParams, CellCallback noFilterCallback, CellCallback filterByTagsCallback, void* pUserData, nsVisibilityState::Enum visType) const;

  /// \brief Candidate for grid caching based on query patterns and filtering efficiency.
  struct CacheCandidate
  {
    nsTagSet m_IncludeTags;                  ///< Tags that must be included for this cached grid
    nsTagSet m_ExcludeTags;                  ///< Tags that must be excluded for this cached grid
    nsSpatialData::Category m_Category;      ///< Spatial data category for this cached grid
    ...
    nsUInt32 m_uiGridIndex = nsInvalidIndex; ///< Index of the associated grid if already cached
  };
