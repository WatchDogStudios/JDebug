...
public:
  ...
  nsGameObjectHandle CreateObject(const nsGameObjectDesc& desc); // [tested]
  ...
  nsGameObjectHandle CreateObject(const nsGameObjectDesc& desc, nsGameObject*& out_pObject); // [tested]
  ...
  void DeleteObjectNow(const nsGameObjectHandle& hObject, bool bAlsoDeleteEmptyParents = true); // [tested]
  ...
  ///
  /// If bAlsoDeleteEmptyParents is set, any ancestor object that has no other children and no components, will also get deleted.
  void DeleteObjectDelayed(const nsGameObjectHandle& hObject, bool bAlsoDeleteEmptyParents = true); // [tested]
  ...
  bool IsValidObject(const nsGameObjectHandle& hObject) const; // [tested]
  ...
  [[nodiscard]] bool TryGetObject(const nsGameObjectHandle& hObject, nsGameObject*& out_pObject); // [tested]

  /// \brief Returns whether an object with the given handle exists and if so writes out the corresponding pointer to out_pObject.
  [[nodiscard]] bool TryGetObject(const nsGameObjectHandle& hObject, const nsGameObject*& out_pObject) const; // [tested]
  ...
  [[nodiscard]] bool TryGetObjectWithGlobalKey(const nsTempHashedString& sGlobalKey, nsGameObject*& out_pObject); // [tested]

  /// \brief Returns whether an object with the given global key exists and if so writes out the corresponding pointer to out_pObject.
  [[nodiscard]] bool TryGetObjectWithGlobalKey(const nsTempHashedString& sGlobalKey, const nsGameObject*& out_pObject) const; // [tested]
  ...
  [[nodiscard]] nsGameObject* SearchForObject(nsStringView sSearchPath, nsGameObject* pReferenceObject = nullptr, const nsRTTI* pExpectedComponent = nullptr); // [tested]

  /// \brief const overload of SearchForObject()
  [[nodiscard]] const nsGameObject* SearchForObject(nsStringView sSearchPath, const nsGameObject* pReferenceObject = nullptr, const nsRTTI* pExpectedComponent = nullptr) const; // [tested]

  /// \brief Returns the total number of objects in this world.
  nsUInt32 GetObjectCount() const; // [tested]

  /// \brief Returns an iterator over all objects in this world in no specific order.
  nsInternal::WorldData::ObjectIterator GetObjects(); // [tested]

  /// \brief Returns an iterator over all objects in this world in no specific order.
  nsInternal::WorldData::ConstObjectIterator GetObjects() const; // [tested]
  ...
  /// \brief Creates an instance of the given module type or derived type or returns a pointer to an already existing instance.
  nsWorldModule* GetOrCreateModule(const nsRTTI* pRtti); // [tested]
  ...
  void PostMessage(const nsGameObjectHandle& hReceiverObject, const nsMessage& msg, nsTime delay, nsObjectMsgQueueType::Enum queueType = nsObjectMsgQueueType::NextFrame) const;
  ...
  void PostMessageRecursive(const nsGameObjectHandle& hReceiverObject, const nsMessage& msg, nsTime delay, nsObjectMsgQueueType::Enum queueType = nsObjectMsgQueueType::NextFrame) const;
  ...
  void PostMessage(const nsComponentHandle& hReceiverComponent, const nsMessage& msg, nsTime delay, nsObjectMsgQueueType::Enum queueType = nsObjectMsgQueueType::NextFrame) const;
  ...
  void GetCoordinateSystem(const nsVec3& vGlobalPosition, nsCoordinateSystem& out_coordinateSystem) const; // [tested]

  /// \brief Sets the coordinate system provider that should be used in this world.
  void SetCoordinateSystemProvider(const nsSharedPtr<nsCoordinateSystemProvider>& pProvider); // [tested]

  /// \brief Returns the coordinate system provider that is associated with this world.
  nsCoordinateSystemProvider& GetCoordinateSystemProvider(); // [tested]

  /// \brief Returns the coordinate system provider that is associated with this world.
  const nsCoordinateSystemProvider& GetCoordinateSystemProvider() const; // [tested]


  /// \brief Returns the clock that is used for all updates in this game world
  nsClock& GetClock(); // [tested]

  /// \brief Returns the clock that is used for all updates in this game world
  const nsClock& GetClock() const; // [tested]
  ...
  /// writing.
  nsInternal::WorldData::ReadMarker& GetReadMarker() const; // [tested]

  /// \brief Mark the world for writing by using NS_LOCK(world.GetWriteMarker()). Only one thread can write at a time.
  nsInternal::WorldData::WriteMarker& GetWriteMarker(); // [tested]
...
private:
  ...
  nsGameObject* Reflection_SearchForObject(nsStringView sSearchPath, nsGameObject* pReferenceObject) { return SearchForObject(sSearchPath, pReferenceObject, nullptr); }
  nsClock* Reflection_GetClock();
  nsRandom* Reflection_GetRandomNumberGenerator();
  ...
  void SetParent(nsGameObject* pObject, nsGameObject* pNewParent,
    nsTransformPreservation::Enum preserve = nsTransformPreservation::Enum::PreserveGlobal);
  ...
  void DeregisterUpdateFunctionInternal(const nsWorldModule::UpdateFunctionDesc& desc);
  void DeregisterUpdateFunctionsInternal(nsWorldModule* pModule);

  void DeleteDeadObjects();
  void DeleteDeadComponents();

  void PatchHierarchyData(nsGameObject* pObject, nsTransformPreservation::Enum preserve);
