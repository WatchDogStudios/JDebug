...
thread_local nsWorldReader::InstantiationContextBase* tl_pReaderContext = nullptr;
...
nsStreamReader& nsWorldReader::GetStream() const
{
  nsWorldReader::InstantiationContext* pContext = ((nsWorldReader::InstantiationContext*)tl_pReaderContext);
...
void nsWorldReader::ReadGameObjectDesc(GameObjectToCreate& godesc)
{
  ...
  *m_pReadStream >> desc.m_bDynamic;
  ...
  desc.m_Tags.Load(*m_pReadStream, nsTagRegistry::GetGlobalRegistry());
...
{
  nsStreamReader& s = *m_pReadStream;
    ...
    for (auto& compTypeInfo : m_ComponentTypes)
    {
      ...
      *m_pReadStream >> uiAllComponentsSize;
      ...
      else
      {
        ...
        {
          const nsUInt64 uiRead = m_pReadStream->ReadBytes(Temp, nsMath::Min<nsUInt32>(uiAllComponentsSize, NS_ARRAY_SIZE(Temp)));
...
nsUniquePtr<nsWorldReader::InstantiationContextBase> nsWorldReader::Instantiate(nsWorld& world, bool bUseTransform, const nsTransform& rootTransform, const nsPrefabInstantiationOptions& options)
{
  ...
  nsUniquePtr<InstantiationContext> pContext = NS_DEFAULT_NEW(InstantiationContext, *this, &world, bUseTransform, rootTransform, options);

  return std::move(pContext);
}

nsWorldReader::InstantiationContext::InstantiationContext(nsWorldReader& ref_worldReader, nsWorld* pWorld, bool bUseTransform, const nsTransform& rootTransform, const nsPrefabInstantiationOptions& options)
  ...
  , m_Options(options)
{
  ...
  {
    ct.m_ComponentIndexToHandle.PushBack(nsComponentHandle());
  }
...
nsWorldReader::InstantiationContext::StepResult nsWorldReader::InstantiationContext::Step()
{
  ...
  NS_LOCK(m_pWorld->GetWriteMarker());
  ...
  if (m_Phase == Phase::CreateComponents)
  {
    if (m_WorldReader.m_ComponentCreationStream.GetStorageSize64() > 0)
    {
      ...
      NS_SCOPE_EXIT(/*m_WorldReader.m_pStream = pPrevReader; */ m_WorldReader.m_pStringDedupReadContext->SetActive(false); tl_pReaderContext = nullptr;);
...
bool nsWorldReader::InstantiationContext::CreateComponents(nsTime endTime)
{
  ...
  nsStreamReader& s = m_CurrentReader;
  ...
  for (; m_uiCurrentComponentTypeIndex < m_WorldReader.m_ComponentTypes.GetCount(); ++m_uiCurrentComponentTypeIndex)
  {
    ...
    nsComponentManagerBase* pManager = m_pWorld->GetOrCreateManagerForComponentType(compTypeInfo.m_pRtti);
    ...
    while (m_uiCurrentIndex < compTypeInfo.m_uiNumComponents)
    {
      ...
      NS_ASSERT_DEBUG(uiComponentIdx == compTypeState.m_ComponentIndexToHandle.GetCount(), "Component index doesn't match");
