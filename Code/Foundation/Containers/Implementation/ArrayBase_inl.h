...
{
  static_assert(nsIsPodType<T>::value == nsTypeIsPod::value, "SetCountUninitialized is only supported for POD types. See NS_DEFINE_AS_POD_TYPE() and NS_DECLARE_POD_TYPE().");
  const nsUInt32 uiOldCount = m_uiCount;
  const nsUInt32 uiNewCount = uiCount;
...
template <typename T, typename Derived>
void nsArrayBase<T, Derived>::InsertRangeAt(nsUInt32 uiIndex, const nsArrayPtr<const T>& range)
{
