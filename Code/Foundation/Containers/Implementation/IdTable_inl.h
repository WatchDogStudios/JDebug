...
template <typename IdType, typename ValueType>
NS_ALWAYS_INLINE typename nsIdTableBase<IdType, ValueType>::IndexType nsIdTableBase<IdType, ValueType>::GetCapacity() const
{
  return m_Capacity;
}
...
template <typename IdType, typename ValueType>
NS_FORCE_INLINE IdType nsIdTableBase<IdType, ValueType>::GetIdUnchecked(const IndexType index) const
{
  NS_ASSERT_DEBUG(index < m_Capacity, "Out of bounds access. Table has {0} elements, trying to access element at index {1}.", m_Capacity, index);
