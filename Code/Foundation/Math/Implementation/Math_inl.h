...
#  endif
  ...
  template <nsUInt32 NumBits>
  constexpr float ColorUnsignedIntToFloat(nsUInt32 value)
  {
    // Implemented according to
    // https://docs.microsoft.com/en-us/windows/desktop/direct3d10/d3d10-graphics-programming-guide-resources-data-conversion
    constexpr nsUInt32 uiMaxValue = nsMath::Bitmask_LowN<nsUInt32>(NumBits);
    constexpr float fMaxValue = static_cast<float>(uiMaxValue);
    return (value & uiMaxValue) * (1.0f / fMaxValue);
  }

  NS_ALWAYS_INLINE constexpr float ColorByteToFloat(nsUInt8 value)
  {
    return ColorUnsignedIntToFloat<8>(value);
  }

  NS_ALWAYS_INLINE constexpr float ColorShortToFloat(nsUInt16 value)
  {
  ...
  template <typename T, typename T2>
  T EvaluateBnsierCurveDerivative(T2 t, const T& startPoint, const T& controlPoint1, const T& controlPoint2, const T& endPoint)
  {
    const T2 mt = 1 - t;
