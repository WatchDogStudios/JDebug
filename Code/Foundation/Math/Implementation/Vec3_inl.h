...
template <typename Type>
NS_IMPLEMENT_IF_FLOAT_TYPE nsVec3Template<Type> nsVec3Template<Type>::MakeOrthogonalVector(const nsVec3Template<Type>& vDirection, const nsVec3Template<Type>& vBasis1, const nsVec3Template<Type>& vBasis2)
{
  NS_ASSERT_DEBUG(vDirection.IsNormalized() && vBasis1.IsNormalized() && vBasis2.IsNormalized(), "All input vectors must be normalized.");

  // do the cross product with the basis that is less similar to the direction
  if (nsMath::Abs(vDirection.Dot(vBasis1)) < nsMath::Abs(vDirection.Dot(vBasis2)))
  {
...
{
  NS_ASSERT_DEBUG(vNormal.IsNormalized(), "The vector to make this vector orthogonal to, must be normalized. It's length is {0}", nsArgF(vNormal.GetLength(), 3));
...
{
  NS_ASSERT_DEBUG(this->IsNormalized(), "This vector must be normalized. Length is: {}", this->GetLength());
  NS_ASSERT_DEBUG(rhs.IsNormalized(), "The other vector must be normalized. Length is: {}", rhs.GetLength());

  return nsMath::ACos(static_cast<float>(nsMath::Clamp(this->Dot(rhs), (Type)-1, (Type)1)));
}

template <typename Type>
nsAngle nsVec3Template<Type>::GetAngleBetween(const nsVec3Template<Type>& vForward, const nsVec3Template<Type>& vUp) const
{
  NS_ASSERT_DEBUG(this->IsNormalized(), "This vector must be normalized. Length is: {}", this->GetLength());
  NS_ASSERT_DEBUG(vForward.IsNormalized(), "The other vector must be normalized. Length is: {}", vForward.GetLength());
  NS_ASSERT_DEBUG(vUp.IsNormalized(), "The other vector must be normalized. Length is: {}", vUp.GetLength());

  const nsVec3 vRight = vForward.CrossRH(vUp).GetNormalized();
  const nsAngle shortAngle = GetAngleBetween(vForward);
