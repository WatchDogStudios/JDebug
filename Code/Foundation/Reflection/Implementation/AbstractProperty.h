...
/// - Variant access: Through nsReflectionUtils for dynamic typing
///
/// Important: If nsPropertyFlags::Pointer is set, you must use GetValuePtr/SetValuePtr instead of casting
/// to nsTypedMemberProperty, because pointer properties have different type semantics.
///
...
/// - nsDynamicArray, nsHybridArray, nsStaticArray
/// - Standard arrays (T[N])
...
///
/// Supported set types:
/// - nsSet (tree-based, ordered)
/// - nsHashSet (hash-based, unordered)
...
/// - Contains(): O(log n) for nsSet, O(1) average for nsHashSet
/// - Insert/Remove: O(log n) for nsSet, O(1) average for nsHashSet
...
///
/// Supported map types:
/// - nsMap (tree-based, ordered by key)
/// - nsHashTable (hash-based, unordered)
...
///
/// Performance characteristics:
/// - GetValue/Contains: O(log n) for nsMap, O(1) average for nsHashTable
/// - Insert/Remove: O(log n) for nsMap, O(1) average for nsHashTable
...
/// - Enums/Bitflags: Passed as nsInt64 values through nsEnum/nsBitflags
/// - Classes: Always passed by pointer regardless of declaration
/// - Out parameters: Written back to source variants after execution
/// - Null values: Represented as invalid variants (except for nsVariant parameters)
///
/// Performance considerations:
