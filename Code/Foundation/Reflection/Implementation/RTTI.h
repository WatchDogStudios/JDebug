...
/// \brief Core class of WDFramework's reflection system that holds complete runtime type information.
///
/// Each nsRTTI instance represents one reflected type and contains all metadata needed for runtime
...
///
/// Usage: Types are typically registered using macros (NS_BEGIN_STATIC_REFLECTED_TYPE, etc.)
/// rather than creating nsRTTI instances manually.
...
///
/// - nsRTTIDefaultAllocator: Standard heap allocation
/// - nsRTTINoAllocator: Prevents dynamic allocation (compile-time types only)
...
/// \brief Standard RTTI allocator that creates instances using WDFramework's allocator system.
///
/// This is the default allocator used by most reflected types. It provides standard heap allocation
/// with proper integration into WDFramework's memory management system. The allocator wrapper allows
...
/// - AllocatorWrapper: Determines which allocator to use (default: nsDefaultAllocatorWrapper)
