...
/// statically reflected types (using macros) and dynamically reflected types (nsReflectedClass).
/// The function automatically detects the reflection method and returns the appropriate RTTI.
///
/// Usage examples:
/// \code
///   const nsRTTI* rtti = nsGetStaticRTTI<MyClass>();
...
///
/// Parameters:
/// - Linkage: Usually NS_FOUNDATION_DLL, NS_CORE_DLL, or NS_NO_LINKAGE for static types
...
///   NS_DECLARE_REFLECTABLE_TYPE(NS_NO_LINKAGE, MyStruct);
/// \endcode
///
/// Note: This is not needed if the type already uses dynamic reflection (nsReflectedClass).
/// Only use for POD types, enums, or simple classes that don't inherit from nsReflectedClass.
...
/// Must be paired with NS_END_STATIC_REFLECTED_TYPE. Between these macros,
/// use NS_BEGIN_PROPERTIES/NS_END_PROPERTIES and similar blocks to define reflection data.
///
/// Parameters:
/// - Type: The type being reflected (must match NS_DECLARE_REFLECTABLE_TYPE)
/// - BaseType: The base class (use nsNoBase if no inheritance)
...
///   - nsRTTIDefaultAllocator<Type>: Standard heap allocation
///   - nsRTTINoAllocator: Disable dynamic creation
///   - Custom allocator: For specialized memory management
///
/// Example:
/// \code
///   // In source file
///   NS_BEGIN_STATIC_REFLECTED_TYPE(MyStruct, nsNoBase, 1, nsRTTIDefaultAllocator<MyStruct>)
///   {
///     NS_BEGIN_PROPERTIES
///     {
///       NS_MEMBER_PROPERTY("value", value)
///     }
///     NS_END_PROPERTIES;
///   }
...
/// Add property macros (NS_MEMBER_PROPERTY, NS_ACCESSOR_PROPERTY, etc.) between this
/// and NS_END_PROPERTIES.
///
/// Example:
/// \code
///   NS_BEGIN_PROPERTIES
...
///     NS_ACCESSOR_PROPERTY("Value", GetValue, SetValue)
///   }
///   NS_END_PROPERTIES;
/// \endcode
...
/// \brief Ends the property declaration block started with NS_BEGIN_PROPERTIES.
