...
template <int N, nsMathAcc::Enum acc>
NS_ALWAYS_INLINE void nsSimdVec4f::NormalizeIfNotZero(const nsSimdVec4f& vFallback, const nsSimdFloat& fEpsilon)
{
  nsSimdVec4b bIsZero = IsZero<N>(fEpsilon);
...
{
  const nsSimdVec4b bIsLessThan = Get<nsSwizzle::YYYY>() < nsSimdVec4f(0.99f);
  return CrossRH(Select(bIsLessThan, nsSimdVec4f(0, 1, 0, 0), nsSimdVec4f(1, 0, 0, 0)));
}

NS_ALWAYS_INLINE const nsSimdVec4f operator*(const nsSimdFloat& f, const nsSimdVec4f& v)
{
  return v * f;
}
