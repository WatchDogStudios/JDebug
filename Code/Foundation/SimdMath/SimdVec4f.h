...
public:
  ...
  template <int N, nsMathAcc::Enum acc = nsMathAcc::FULL>
  void NormalizeIfNotZero(const nsSimdVec4f& vFallback, const nsSimdFloat& fEpsilon = nsMath::SmallEpsilon<float>()); // [tested]
...
const nsSimdVec4f operator*(const nsSimdFloat& f, const nsSimdVec4f& v);
