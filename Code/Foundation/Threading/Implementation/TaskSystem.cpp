...
void nsTaskSystem::BroadcastClearThreadLocalsEvent()
{
  for (nsUInt32 i = 0; i < nsWorkerThreadType::ENUM_COUNT; ++i)
  {
    for (nsTaskWorkerThread* pWorker : s_pThreadState->m_Workers[i])
    {
      if (pWorker)
      {
