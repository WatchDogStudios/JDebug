...
nsResult nsImageView::SaveTo(nsStringView sFileName) const
{
  ...
  if (const nsImageFileFormat* pFormat = nsImageFileFormat::GetWriterFormat(it))
  {
    if (pFormat->WriteImage(writer, *this, it) != NS_SUCCESS)
    {
...
nsResult nsImage::LoadFrom(nsStringView sFileName)
{
  ...
  {
    if (pFormat->ReadImage(reader, *this, it) != NS_SUCCESS)
    {
