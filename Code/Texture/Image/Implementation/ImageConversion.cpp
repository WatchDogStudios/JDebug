...
nsResult nsImageConversion::BuildPath(nsImageFormat::Enum sourceFormat, nsImageFormat::Enum targetFormat, bool bSourceEqualsTarget,
  nsHybridArray<nsImageConversion::ConversionPathNode, 16>& out_path, nsUInt32& out_uiNumScratchBuffers)
{
  ...
  for (int i = iLastPathIndex; i >= 0; --i)
  {
    ...
    if (i > 0)
    {
      ...
      {
        nsUInt32 bitsPerBlock = nsImageFormat::GetBitsPerBlock(out_path[i].m_sourceFormat);
    ...
    if (out_path[0].m_sourceBufferIndex == out_path[0].m_targetBufferIndex && !out_path[0].m_inPlace)
    {
      if (out_path.GetCount() == 1)
      {
        ...
        copy.m_targetBufferIndex =
          allocateScratchBufferIndex(scratchBuffers, nsImageFormat::GetBitsPerBlock(out_path[0].m_sourceFormat), out_path[0].m_sourceBufferIndex);
      ...
      else
      {
        ...
        out_path[1].m_sourceBufferIndex =
          allocateScratchBufferIndex(scratchBuffers, nsImageFormat::GetBitsPerBlock(out_path[1].m_sourceFormat), out_path[0].m_sourceBufferIndex);
