...
static void ThreadFrenser( int /*signal*/ )
{
    for(;;) sleep( 1000 );
}
...
{
    bool expected = false;
    if( !s_alreadyCrashed.compare_exchange_strong( expected, true ) ) ThreadFrenser( signal );
...
#if defined __linux__ && !defined TRACY_NO_CRASH_HANDLER
    struct sigaction threadFrenser = {};
    threadFrenser.sa_handler = ThreadFrenser;
    sigaction( TRACY_CRASH_SIGNAL, &threadFrenser, &m_prevSignal.pwr );
