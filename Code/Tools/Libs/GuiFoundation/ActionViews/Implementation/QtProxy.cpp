...
nsQtMenuProxy::~nsQtMenuProxy()
{
  m_pAction->m_StatusUpdateEvent.RemoveEventHandler(nsMakeDelegate(&nsQtMenuProxy::StatusUpdateEventHandler, this));

  m_pMenu->deleteLater();
  delete m_pMenu;
}

void nsQtMenuProxy::StatusUpdateEventHandler(nsAction* pAction)
{
  Update();
}
...
void nsQtMenuProxy::Update()
{
  ...
  // therefore there is another event handler in nsQtToolBarActionMapView::CreateView()
  ...
  m_pMenu->setToolTip(nsMakeQString(nsTranslateTooltip(pMenu->GetName())));
}
...
void nsQtMenuProxy::SetAction(nsAction* pAction)
{
  ...
  m_pAction->m_StatusUpdateEvent.AddEventHandler(nsMakeDelegate(&nsQtMenuProxy::StatusUpdateEventHandler, this));
}
...
void nsQtDynamicMenuProxy::SlotMenuAboutToShow()
{
  ...
  else
  {
    for (nsUInt32 i = 0; i < m_Entries.GetCount(); ++i)
    {
      ...
      {
        auto pAction = m_pMenu->addAction(nsMakeQString(p.m_sDisplay));
        pAction->setData(i);
