  ...
  for (auto pChild : pObject->GetChildren())
  {
    ...
    switch (pDesc->m_hAction.GetDescriptor()->m_Type)
    {
      ...
      case nsActionType::Menu:
      {
        ...
        nsStringBuilder sTooltip = nsTranslateTooltip(pNamed->GetName());
        if (sTooltip.IsEmpty())
        {
          sTooltip = nsTranslate(pNamed->GetName());
          sTooltip.ReplaceAll("&", "");
        }
        pButton->setToolTip(nsMakeQString(sTooltip));

        pNamed->m_StatusUpdateEvent.AddEventHandler([=](nsAction* pAction)
          { pButton->setIcon(nsQtUiServices::GetCachedIconResource(pNamed->GetIconPath())); });
