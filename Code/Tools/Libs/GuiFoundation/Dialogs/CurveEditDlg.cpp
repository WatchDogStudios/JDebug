...
void nsQtCurveEditDlg::RetrieveCurveState()
{
  ...
  {
    const nsDocumentObject* pPoint = m_pObjectAccessor->GetChildObjectByName(m_pCurveObject, "ControlPoints", i);
...
void nsQtCurveEditDlg::OnCpMovedEvent(nsUInt32 curveIdx, nsUInt32 cpIdx, nsInt64 iTickX, double newPosY)
{
  ...
  {
    const nsDocumentObject* pPoint = m_pObjectAccessor->GetChildObjectByName(m_pCurveObject, "ControlPoints", cpIdx);
...
void nsQtCurveEditDlg::OnTangentMovedEvent(nsUInt32 curveIdx, nsUInt32 cpIdx, float newPosX, float newPosY, bool rightTangent)
{
  ...
  // update the actual object
  {
    ...
    if (rightTangent)
      m_pObjectAccessor->SetValueByName(pPoint, "RightTangent", nsVec2(newPosX, newPosY)).AssertSuccess();
    else
      m_pObjectAccessor->SetValueByName(pPoint, "LeftTangent", nsVec2(newPosX, newPosY)).AssertSuccess();
  }
}
...
void nsQtCurveEditDlg::OnInsertCpEvent(nsUInt32 curveIdx, nsInt64 tickX, double value)
{
  ...
  {
    nsUuid guid;
    m_pObjectAccessor->AddObjectByName(m_pCurveObject, "ControlPoints", -1, nsGetStaticRTTI<nsCurveControlPointData>(), guid).AssertSuccess();
