...
{
  NS_LOG_BLOCK("DocSlotRestoreLayout");
...
void nsQtDocumentWindow::SaveWindowLayout()
{
  ...
  nsLog::Debug("Save Layout - {}", GetDocument()->GetDocumentTypeName());
...
void nsQtDocumentWindow::RestoreWindowLayout(bool bForce)
{
  if (GetDocument() == nullptr)
    return;
  ...
  if (GetDocument() != nullptr)
  {
    nsLog::Debug("Restore Layout - {} ({})", GetDocument()->GetDocumentTypeName(), GetDocument()->GetDocumentPath());
  }
