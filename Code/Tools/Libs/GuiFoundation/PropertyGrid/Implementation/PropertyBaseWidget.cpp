    ...
    connect(pRevert, &QAction::triggered, this, [this]()
      ...
      switch (m_pProp->GetCategory())
      {
        ...
        case nsPropertyCategory::Enum::Map:
        {
          ...
          {
            // Revert container
            nsDefaultContainerState defaultState(m_pType, m_pObjectAccessor, m_Items, m_pProp->GetPropertyName());
          ...
          else
          {
            ...
            else
            {
              ...
              nsDefaultObjectState defaultState(m_pType, m_pObjectAccessor, ResolvedObjects);
        ...
        default:
        {
          ...
          nsDefaultObjectState defaultState(m_pType, m_pObjectAccessor, m_Items);
  ...
  : nsQtPropertyWidget()
{
  ...
  m_pAddButton = new nsQtAddSubElementButton(nsPropertyCategory::Member, nsTranslate("POINTERWIDGET_Create"));
...
nsQtPropertyContainerWidget::Element& nsQtPropertyContainerWidget::AddElement(nsUInt32 index)
{
  ...
  // Add Buttons
  auto pAttr = m_pProp->GetAttributeByType<nsContainerAttribute>();
  if ((!pAttr || pAttr->CanMove()) && GetContainerCategory() != nsPropertyCategory::Map)
  {
...
void nsQtPropertyContainerWidget::UpdateElements()
{
  ...
  const nsUInt32 iElements = m_Keys.GetCount();
...
void nsQtPropertyContainerWidget::GetRequiredElements(nsDynamicArray<nsVariant>& out_keys) const
{
  out_keys.Clear();
  if (GetContainerCategory() == nsPropertyCategory::Map)
  {
    NS_VERIFY(m_pObjectAccessor->GetKeys(m_Items[0].m_pObject, m_pProp, out_keys).Succeeded(), "GetKeys should always succeed.");
    nsHybridArray<nsVariant, 16> keys;
    for (nsUInt32 i = 1; i < m_Items.GetCount(); i++)
    {
      keys.Clear();
      NS_VERIFY(m_pObjectAccessor->GetKeys(m_Items[i].m_pObject, m_pProp, keys).Succeeded(), "GetKeys should always succeed.");
    ...
    out_keys.Sort([](const nsVariant& a, const nsVariant& b)
  ...
  else
  {
    ...
    {
      nsInt32 iCount = 0;
      NS_VERIFY(m_pObjectAccessor->GetCount(item.m_pObject, m_pProp, iCount).Succeeded(), "GetCount should always succeed.");
...
nsPropertyCategory::Enum nsQtPropertyContainerWidget::GetContainerCategory() const
{
...
void nsQtPropertyContainerWidget::OnInit()
{
  ...
  {
    nsStringBuilder sTmp, tmp2;
    sTmp.SetFormat(nsTranslate("CONTAINER_AddEntry").GetData(tmp2), m_pProp->GetPropertyName());

    m_pAddButton = new nsQtAddSubElementButton(GetContainerCategory(), sTmp);
    m_pAddButton->Init(m_pGrid, m_pObjectAccessor, m_pType, m_pProp);
...
{
  NS_ASSERT_DEV(GetContainerCategory() != nsPropertyCategory::Map, "Map entries can't be moved.");
...
nsQtPropertyTypeContainerWidget::nsQtPropertyTypeContainerWidget() = default;
...
void nsQtVariantPropertyWidget::InternalSetValue(const nsVariant& value)
{
  ...
  if (pNewtSubType != m_pCurrentSubType || m_pWidget == nullptr)
  {
    ...
    if (pNewtSubType)
    {
      if (commonType == nsVariantType::VariantArray || commonType == nsVariantType::VariantDictionary)
      {
        m_pWidget = new nsQtVariantContainerWidget(commonType);
      }
      else
...
void nsQtVariantPropertyWidget::ChangeVariantType(nsVariantType::Enum type)
{
  ...
  for (const auto& item : m_Items)
  {
    ...
    {
      NS_VERIFY(m_pObjectAccessor->SetValue(item.m_pObject, m_pProp, nsReflectionUtils::GetDefaultVariantFromType(type), item.m_Index).Succeeded(), "");
    }
  }
  m_pObjectAccessor->FinishTransaction();
}

void nsQtVariantPropertyWidget::EnableTypeSelection(bool bEnable)
{
...
{
  if (type != nsVariantType::VariantArray && type != nsVariantType::VariantDictionary)
  {
...
/// *** nsQtVariantContainerWidget ***

nsQtVariantContainerWidget::nsQtVariantContainerWidget(nsVariantType::Enum variantType)
{
  switch (variantType)
  {
    case nsVariantType::VariantArray:
      m_ContainerCategory = nsPropertyCategory::Array;
      break;
    case nsVariantType::VariantDictionary:
      m_ContainerCategory = nsPropertyCategory::Map;
      break;
    default:
      NS_REPORT_FAILURE("Only VariantArray and VariantDictionary are supported by nsQtVariantContainerWidget.");
  }
}

void nsQtVariantContainerWidget::OnInit()
{
  // Init is only called once at creation time so it is safe to replace the object accessor here.
  // As each nsVariantSubAccessor manages only one depth level into the nsVariant we need to wrap the object accessor for each level again which requires creating a unique accessor for each container and maintaining ownership to it.
  m_pVariantSubAccessor = NS_DEFAULT_NEW(nsVariantSubAccessor, m_pObjectAccessor, m_pProp);
  m_pObjectAccessor = m_pVariantSubAccessor.Borrow();
  nsQtPropertyContainerWidget::OnInit();
}

void nsQtVariantContainerWidget::SetSelection(const nsHybridArray<nsPropertySelection, 8>& items)
{
  nsMap<const nsDocumentObject*, nsVariant> subItems;
  for (auto it : items)
  {
    subItems.Insert(it.m_pObject, it.m_Index);
  }
  m_pVariantSubAccessor->SetSubItems(subItems);
  nsQtPropertyContainerWidget::SetSelection(items);
}

nsPropertyCategory::Enum nsQtVariantContainerWidget::GetContainerCategory() const
{
