...
void nsQtPropertyEditorDoubleSpinboxWidget::InternalSetValue(const nsVariant& value)
{
  ...
  if (m_OriginalType == nsVariantType::Invalid)
  {
  ...
  if (m_OriginalType == nsVariantType::Invalid)
  {
    m_OriginalType = nsVariantType::Double;
  }
...
void nsQtPropertyEditorIntSpinboxWidget::InternalSetValue(const nsVariant& value)
{
  ...
  const nsRTTI* type = prop->GetSpecificType();
  ...
  if (m_OriginalType == nsVariantType::Invalid)
  {
    m_OriginalType = nsVariantType::Int32;
  }
...
void nsQtPropertyEditorIntSpinboxWidget::onBeginTemporary()
{
  if (m_bUseTemporaryTransaction && !m_bTemporaryCommand)
  {
    Broadcast(nsPropertyEvent::Type::BeginTemporary);
...
void nsQtPropertyEditorIntSpinboxWidget::onEndTemporary()
{
  if (m_bTemporaryCommand)
    Broadcast(nsPropertyEvent::Type::EndTemporary);
...
void nsQtPropertyEditorSliderWidget::onBeginTemporary()
{
  if (!m_bTemporaryCommand)
  {
    Broadcast(nsPropertyEvent::Type::BeginTemporary);
...
void nsQtPropertyEditorSliderWidget::onEndTemporary()
{
  if (m_bTemporaryCommand)
    Broadcast(nsPropertyEvent::Type::EndTemporary);
...
nsQtPropertyEditorTransformWidget::nsQtPropertyEditorTransformWidget()
  : nsQtStandardPropertyWidget()
{
  ...
  nsUInt32 uiCurrentWidget = 0;
  auto AddWidget = [&](QLayout* layout, double fStep, const char* szLabel, const char* szTooltip, nsColorGammaUB color, const char* szDisplaySuffix)
  {
    auto pSpinBox = new nsQtDoubleSpinBox(this);
    pSpinBox->installEventFilter(this);
    pSpinBox->setMinimum(-nsMath::Infinity<double>());
    pSpinBox->setMaximum(nsMath::Infinity<double>());
...
void nsQtPropertyEditorTransformWidget::OnInit() {}

void nsQtPropertyEditorTransformWidget::InternalSetValue(const nsVariant& value)
{
  if (m_bTemporaryCommand)
    return;
  ...
  nsQtScopedBlockSignals b1(m_pWidget[1]);
  nsQtScopedBlockSignals b2(m_pWidget[2]);
  nsQtScopedBlockSignals b3(m_pWidget[3]);
  nsQtScopedBlockSignals b4(m_pWidget[4]);
  nsQtScopedBlockSignals b5(m_pWidget[5]);
  nsQtScopedBlockSignals b6(m_pWidget[6]);
  nsQtScopedBlockSignals b7(m_pWidget[7]);
  nsQtScopedBlockSignals b8(m_pWidget[8]);

  if (value.IsValid())
  {
    const nsTransform t = value.ConvertTo<nsTransform>();
  ...
  {
    for (nsUInt32 i = 0; i < NS_ARRAY_SIZE(m_pWidget); ++i)
    {
...
void nsQtPropertyEditorTransformWidget::on_EditingFinished_triggered()
{
  if (m_bTemporaryCommand)
    Broadcast(nsPropertyEvent::Type::EndTemporary);
...
void nsQtPropertyEditorTransformWidget::SlotValueChanged()
{
  if (!m_bTemporaryCommand)
  {
    Broadcast(nsPropertyEvent::Type::BeginTemporary);
  ...
  nsAngle x = nsAngle::MakeFromDegree(m_pWidget[3]->value());
  nsAngle y = nsAngle::MakeFromDegree(m_pWidget[4]->value());
  nsAngle z = nsAngle::MakeFromDegree(m_pWidget[5]->value());
  t.m_qRotation = nsQuat::MakeFromEulerAngles(x, y, z);
...
void nsQtPropertyEditorLineEditWidget::InternalSetValue(const nsVariant& value)
{
  ...
  if (m_OriginalType == nsVariantType::Invalid)
  {
    m_OriginalType = nsVariantType::String;
  }
...
void nsQtColorButtonWidget::SetColor(const nsVariant& color)
{
  ...
  {
    const nsColorGammaUB col = nsColor::LightGrey;
...
void nsQtPropertyEditorColorWidget::OnInit()
{
  ...
  m_bExposeAlpha |= (m_pProp->GetSpecificType() == nsGetStaticRTTI<nsVariant>());
}
...
void nsQtPropertyEditorColorWidget::InternalSetValue(const nsVariant& value)
{
  ...
  m_bIsHDR = value.GetType() == nsVariantType::Color;
}
...
void nsQtPropertyEditorColorWidget::on_Button_triggered()
{
  ...
  nsQtUiServices::GetSingleton()->ShowColorDialog(temp, m_bExposeAlpha, m_bIsHDR, this, SLOT(on_CurrentColor_changed(const nsColor&)), SLOT(on_Color_accepted()), SLOT(on_Color_reset()));
}
...
void nsQtCurve1DButtonWidget::UpdatePreview(nsObjectAccessorBase* pObjectAccessor, const nsDocumentObject* pCurveObject, QColor color, double fLowerExtents, bool bLowerFixed, double fUpperExtents, bool bUpperFixed, double fDefaultValue, double fLowerRange, double fUpperRange)
{
  ...
  {
    const nsDocumentObject* pPoint = pObjectAccessor->GetChildObjectByName(pCurveObject, "ControlPoints", i);
...
void nsQtPropertyEditorCurve1DWidget::UpdatePreview()
{
  ...
  const nsDocumentObject* pCurve = m_pObjectAccessor->GetChildObjectByName(pParent, m_pProp->GetPropertyName(), {});
