...
public:
  ...
  nsQtPropertyGridWidget* GetPropertyGrid() { return m_pGrid; }
  nsObjectAccessorBase* GetObjectAccessor() { return m_pObjectAccessor; }
...
protected:
  ...
  void UpdateElements();
  virtual void GetRequiredElements(nsDynamicArray<nsVariant>& out_keys) const;
  virtual void UpdatePropertyMetaState();
  /// \brief Some containers like nsVariant can be both a map or an array so we can't reply on the property type alone. For these containers, this method can be overwritten to retrieve the category from something other than `m_pProp->GetCategory()`.
  virtual nsPropertyCategory::Enum GetContainerCategory() const;
...
protected:
  ...
  nsHybridArray<nsVariant, 16> m_Keys;
...
// Used for sub-containers of an nsVariant, e.g. an nsVariantArray or nsVariantDictionary stored inside an nsVariant. nsVariantSubAccessor is used to create a view into a sub-tree container of the nsVariant.
class NS_GUIFOUNDATION_DLL nsQtVariantContainerWidget : public nsQtPropertyStandardTypeContainerWidget
{
  Q_OBJECT;

public:
  nsQtVariantContainerWidget(nsVariantType::Enum variantType);
  virtual ~nsQtVariantContainerWidget() = default;

protected:
  virtual void OnInit() override;
  virtual void SetSelection(const nsHybridArray<nsPropertySelection, 8>& items) override;
  virtual nsPropertyCategory::Enum GetContainerCategory() const override;

private:
  nsUniquePtr<nsVariantSubAccessor> m_pVariantSubAccessor;
