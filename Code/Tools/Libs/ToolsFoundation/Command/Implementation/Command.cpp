...
nsStatus nsCommand::Do(bool bRedo)
{
  ...
  if (status.Failed())
  {
    ...
    else
    {
      for (nsInt32 j = m_ChildActions.GetCount() - 1; j >= 0; --j)
      {
        ...
        NS_ASSERT_DEV(status2.Succeeded(), "Failed do could not be recovered! Inconsistent state!");
      }
      return status;
    }
  }
  ...
  for (nsUInt32 i = 0; i < uiChildActions; ++i)
  {
    ...
    if (status.Failed())
    {
      ...
      {
        nsStatus status2 = m_ChildActions[j]->Undo(true);
        NS_ASSERT_DEV(status2.Succeeded(), "Failed redo could not be recovered! Inconsistent state!");
      }
      // A command that originally succeeded failed on redo!
      return status;
    }
  ...
  for (nsInt32 i = uiChildActions - 1; i >= 0; --i)
  {
    ...
    if (status.Failed())
    {
      for (nsUInt32 j = i + 1; j < uiChildActions; ++j)
      {
        ...
        NS_ASSERT_DEV(status2.Succeeded(), "Failed undo could not be recovered! Inconsistent state!");
      }
