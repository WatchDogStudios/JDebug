...
{
  nsDocument* m_pDocument = nullptr; ///< The document in which the accessor change occurred.
  nsObjectAccessorBase* m_pOldObjectAccessor = nullptr;
  nsObjectAccessorBase* m_pNewObjectAccessor = nullptr;
};
...
protected:
  nsDocument* m_pHostDocument = nullptr; ///< Pointer to the main document if this is a sub-document, otherwise self.
  nsDocument* m_pActiveSubDocument = nullptr; ///< Pointer to the currently active sub-document.
...
public:
  ...
  struct PasteInfo
  {
    ...
    nsDocumentObject* m_pObject = nullptr; ///< The object being pasted.
    nsDocumentObject* m_pParent = nullptr; ///< The parent object to paste into.
    nsInt32 m_Index = -1;                  ///< The index at which to insert the object.
  };
...
private:
  ...
  nsUInt32 m_uiUnknownObjectTypeInstances = 0;

  nsTaskGroupID m_ActiveSaveTask;
  nsStatus m_LastSaveResult = NS_SUCCESS;
};
