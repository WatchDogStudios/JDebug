...
void nsDocumentNodeManager::Connect(const nsDocumentObject* pObject, const nsPin& source, const nsPin& target)
{
  ...
  NS_ASSERT_DEBUG(CanConnect(pObject->GetType(), source, target, res).Succeeded(), "Connect: Sanity check failed!");
...
void nsDocumentNodeManager::Disconnect(const nsDocumentObject* pObject)
{
  ...
  NS_ASSERT_DEBUG(CanDisconnect(pObject).Succeeded(), "Disconnect: Sanity check failed!");
...
{
  NS_ASSERT_DEBUG(CanMoveNode(pObject, vPos).Succeeded(), "MoveNode: Sanity check failed!");
