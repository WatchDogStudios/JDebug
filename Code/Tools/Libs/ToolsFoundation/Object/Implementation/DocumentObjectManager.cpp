...
void nsDocumentObjectManager::AddObject(nsDocumentObject* pObject, nsDocumentObject* pParent, nsStringView sParentProperty, nsVariant index)
{
  ...
  NS_ASSERT_DEV(CanAdd(pObject->GetTypeAccessor().GetType(), pParent, sParentProperty, index).Succeeded(), "Trying to execute invalid add!");
...
{
  NS_ASSERT_DEV(CanRemove(pObject).Succeeded(), "Trying to execute invalid remove!");
...
{
  NS_ASSERT_DEV(CanMove(pObject, pNewParent, sParentProperty, index).Succeeded(), "Trying to execute invalid move!");
