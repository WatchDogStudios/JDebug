...
nsEvent<const nsToolsProjectEvent&, nsMutex> nsToolsProject::s_Events;
...
nsStatus nsToolsProject::Create()
{
  ...
  NS_SUCCEED_OR_RETURN(Open());

  // if this file already exists, the project was created from a template and should not get additional setup
  nsStringBuilder path(GetProjectDirectory(), "/Scenes/Main.nsScene");
  if (!nsOSFile::ExistsFile(path))
  {
    nsToolsProjectEvent e;
    e.m_pProject = this;
    e.m_Type = nsToolsProjectEvent::Type::ProjectFirstSetup;
    s_Events.Broadcast(e);
  }
...
{
  nsStringBuilder s = GetProjectDirectory();
  s.AppendPath("Editor");
