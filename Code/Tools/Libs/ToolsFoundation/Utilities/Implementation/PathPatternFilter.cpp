...
bool nsPathPatternFilter::PassesFilters(nsStringView sText, nsStringBuilder* pMatchingFilter) const
{
  for (const auto& filter : m_IncludePatterns)
  {
    ...
    if (filter.Matches(sText))
    {
      if (pMatchingFilter)
      {
        ...
        if (filter.m_MatchType == nsPathPattern::EndsWith || filter.m_MatchType == nsPathPattern::Contains)
          pMatchingFilter->Append("*");

        pMatchingFilter->Append(filter.m_sString);

        if (filter.m_MatchType == nsPathPattern::StartsWith || filter.m_MatchType == nsPathPattern::Contains)
          pMatchingFilter->Append("*");
      }
