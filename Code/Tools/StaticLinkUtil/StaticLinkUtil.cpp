...
StaticLinkUtil.exe "C:\WDFramework\Trunk\Code\Engine\Foundation"
...
public:
  ...
  void FindIncludes(nsStringBuilder& ref_sFileContent)
  {
    ...
    while (true)
    {
      ...
      if (nsStringUtils::IsEqualN(szI, "#include", 8))
      {
        ...
        // ignore includes to files that cannot be found (ie. they are not part of the WDFramework source tree)
  ...
  void IterateOverFiles()
  {
    ...
    for (const nsString& sFile : m_FilesToModify)
    {
      ...
      {
        NS_LOG_BLOCK("Header", sFile.GetFileNameAndExtension());
      ...
      {
        NS_LOG_BLOCK("Source", sFile.GetFileNameAndExtension());
        FixFileContents(sFile);
