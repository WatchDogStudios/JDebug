...
NS_CREATE_SIMPLE_TEST(Containers, IdTable)
{
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Constructor")
  {
    ...
    NS_TEST_BOOL(table.GetCapacity() == 0);
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Copy Constructor/Assignment/Iterator")
  {
    NS_TEST_BOOL(st::HasAllDestructed());
    {
      ...
      NS_TEST_BOOL(table1.GetCapacity() >= 100);
      NS_TEST_BOOL(table2.GetCapacity() >= 100);
      NS_TEST_BOOL(table3.GetCapacity() >= 100);
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Insert/Remove")
  {
    ...
    nsUInt32 uiCapacity = table.GetCapacity();
    ...
    NS_TEST_INT_MSG(table.GetCapacity(), uiCapacity, "Removing items should not decrease capacity");
    for (int i = 0; i < 40; i++)
    {
