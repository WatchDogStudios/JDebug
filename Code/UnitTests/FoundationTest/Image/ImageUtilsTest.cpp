...
NS_CREATE_SIMPLE_TEST(Image, ImageUtils)
{
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Scaleup 16x RGBA")
  {
    nsImage ImageA, ImageUpscaled;
    ImageA.LoadFrom("ImageUtils/ColoredChecker.tga").IgnoreResult();

    nsImageUtils::Scale(ImageA, ImageUpscaled, ImageA.GetWidth() * 16, ImageA.GetHeight() * 16, nullptr, nsImageAddressMode::Repeat, nsImageAddressMode::Repeat).IgnoreResult();

    ImageUpscaled.SaveTo(":output/ImageUtils/ColoredChecker16x.tga").IgnoreResult();

    NS_TEST_FILES("ImageUtils/ExpectedColoredChecker16x.tga", "ImageUtils/ColoredChecker16x.tga", "");
  }
