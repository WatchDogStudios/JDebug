...
NS_CREATE_SIMPLE_TEST(Math, Vec3)
{
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "GetAngleBetween")
  {
    ...
    v.Set(1, 0, 0);
    NS_TEST_FLOAT(v.GetAngleBetween(nsVec3(1, 0, 0), nsVec3(0, 0, 1)).GetDegree(), 0.0f, 0.001f);

    v.Set(1, -1, 0);
    v.Normalize();
    NS_TEST_FLOAT(v.GetAngleBetween(nsVec3(1, 0, 0), nsVec3(0, 0, 1)).GetDegree(), 45.0f, 0.001f);
    ...
    v.Set(-1, 0, 0);
    v.Normalize();
    NS_TEST_FLOAT(v.GetAngleBetween(nsVec3(1, 0, 0), nsVec3(0, 0, 1)).GetDegree(), 180.0f, 0.001f);

    v.Set(1, 0, 0);
    v.Normalize();
    NS_TEST_FLOAT(v.GetAngleBetween(nsVec3(0, 1, 0), nsVec3(0, 0, 1)).GetDegree(), 90.0f, 0.001f);

    v.Set(0, 1, 0);
    v.Normalize();
    NS_TEST_FLOAT(v.GetAngleBetween(nsVec3(1, 0, 0), nsVec3(0, 0, 1)).GetDegree(), -90.0f, 0.001f);
  }
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "MakeRandomDeviation")
  {
    NS_TEST_VEC3(nsVec3T::MakeOrthogonalVector(nsVec3(1, 0, 0)), nsVec3T(0, 0, 1), 0.001f);
    NS_TEST_VEC3(nsVec3T::MakeOrthogonalVector(nsVec3(0, 1, 0)), nsVec3T(0, 0, -1), 0.001f);
    NS_TEST_VEC3(nsVec3T::MakeOrthogonalVector(nsVec3(0, 0, 1)), nsVec3T(-1, 0, 0), 0.001f);

    nsVec3 dir;

    dir.Set(1, 2, 3);
    dir.Normalize();
    NS_TEST_FLOAT(dir.Dot(nsVec3T::MakeOrthogonalVector(dir)), 0.0f, 0.001f);
