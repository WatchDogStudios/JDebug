...
NS_CREATE_SIMPLE_TEST(Time, Clock)
{
  ...
  NS_TEST_BLOCK(nsTestBlock::Enabled, "SetMinimumTimeStep / GetMinimumTimeStep")
  {
    ...
    double smallestValue = nsMath::MaxValue<double>();
    for (nsUInt32 i = 0; i < 10; i++)
    {
      c.Update();
      smallestValue = nsMath::Min(smallestValue, c.GetTimeDiff().GetSeconds());
      if (nsMath::IsEqual(smallestValue, c.GetMinimumTimeStep().GetSeconds(), 0.0000000001))
        break;
    }
    NS_TEST_DOUBLE_MSG(smallestValue, c.GetMinimumTimeStep().GetSeconds(), 0.0000000001, "After 10 itterations, c.GetTimeDiff() did not reach the minimum step");
