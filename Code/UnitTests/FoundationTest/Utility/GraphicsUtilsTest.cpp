...
NS_CREATE_SIMPLE_TEST(Utility, GraphicsUtils)
{
  NS_TEST_BLOCK(nsTestBlock::Enabled, "Perspective (-1/1): ConvertWorldPosToScreenPos / ConvertScreenPosToWorldPos")
  {
    ...
    mProj = nsGraphicsUtils::CreatePerspectiveProjectionMatrixFromFovX(nsAngle::MakeFromDegree(85.0f), 2.0f, 1.0f, 1000.0f, nsClipSpaceDepthRange::MinusOneToOne, nsClipSpaceYMode::Regular, nsHandedness::LeftHanded);
    ...
    for (nsUInt32 y = 0; y < 25; ++y)
    {
      ...
      {
        nsVec3 vPoint, vDir;
        NS_TEST_BOOL(nsGraphicsUtils::ConvertScreenPosToWorldPos(mProjInv, 0, 0, 50, 25, nsVec3((float)x, (float)y, 0.5f), vPoint, &vDir, nsClipSpaceDepthRange::MinusOneToOne).Succeeded());

        NS_TEST_VEC3(vDir, vPoint.GetNormalized(), 0.01f);

        nsVec3 vScreen;
        NS_TEST_BOOL(nsGraphicsUtils::ConvertWorldPosToScreenPos(mProj, 0, 0, 50, 25, vPoint, vScreen, nsClipSpaceDepthRange::MinusOneToOne).Succeeded());
  ...
  {
    nsMat4 mProj, mProjInv;
    mProj = nsGraphicsUtils::CreatePerspectiveProjectionMatrixFromFovX(nsAngle::MakeFromDegree(85.0f), 2.0f, 1.0f, 1000.0f, nsClipSpaceDepthRange::ZeroToOne, nsClipSpaceYMode::Regular, nsHandedness::LeftHanded);
    mProjInv = mProj.GetInverse();
    ...
    for (nsUInt32 y = 0; y < 25; ++y)
    {
      ...
      {
        nsVec3 vPoint, vDir;
        NS_TEST_BOOL(nsGraphicsUtils::ConvertScreenPosToWorldPos(mProjInv, 0, 0, 50, 25, nsVec3((float)x, (float)y, 0.5f), vPoint, &vDir, nsClipSpaceDepthRange::ZeroToOne).Succeeded());
  ...
  {
    nsMat4 mProj, mProjInv;
    mProj = nsGraphicsUtils::CreateOrthographicProjectionMatrix(50, 25, 1.0f, 1000.0f, nsClipSpaceDepthRange::MinusOneToOne, nsClipSpaceYMode::Regular, nsHandedness::LeftHanded);
  ...
  {
    nsMat4 mProj, mProjInv;
    mProj = nsGraphicsUtils::CreateOrthographicProjectionMatrix(50, 25, 1.0f, 1000.0f, nsClipSpaceDepthRange::ZeroToOne, nsClipSpaceYMode::Regular, nsHandedness::LeftHanded);
    mProjInv = mProj.GetInverse();
