...
void RecursiveModifyProperty(const nsDocumentObject* pObject, const nsAbstractProperty* pProp, nsObjectAccessorBase* pObjectAccessor)
{
  if (pProp->GetCategory() == nsPropertyCategory::Member)
  {
    if (pProp->GetFlags().IsSet(nsPropertyFlags::Pointer))
    {
      if (pProp->GetFlags().IsSet(nsPropertyFlags::PointerOwner))
      {
        ...
        if (oldGuid.IsValid())
        {
          NS_TEST_BOOL(pObjectAccessor->RemoveObject(pObjectAccessor->GetObject(oldGuid)).Succeeded());
        }

        NS_TEST_BOOL(pObjectAccessor->AddObject(pObject, pProp, nsVariant(), pProp->GetSpecificType(), newGuid).Succeeded());
      ...
      else
      {
        ...
        NS_TEST_BOOL(pObjectAccessor->SetValue(pObject, pProp, value).Succeeded());
      }
    }
    else
    {
  ...
  else if (pProp->GetCategory() == nsPropertyCategory::Array || pProp->GetCategory() == nsPropertyCategory::Set)
  {
        ...
        !pProp->GetFlags().IsSet(nsPropertyFlags::PointerOwner))
    {
      ...
      NS_TEST_BOOL(pObjectAccessor->InsertValue(pObject, pProp, value1, 0).Succeeded());
      NS_TEST_BOOL(pObjectAccessor->InsertValue(pObject, pProp, value2, 1).Succeeded());
    }
    else if (pProp->GetFlags().IsSet(nsPropertyFlags::Class))
    {
      ...
      for (nsInt32 i = iCurrentCount - 1; i >= 0; --i)
      {
        NS_TEST_BOOL(pObjectAccessor->RemoveObject(pObjectAccessor->GetObject(currentValues[i].Get<nsUuid>())).Succeeded());
      }

      if (pProp->GetCategory() == nsPropertyCategory::Array)
      {
        nsUuid newGuid = nsUuid::MakeUuid();
        NS_TEST_BOOL(pObjectAccessor->AddObject(pObject, pProp, 0, pProp->GetSpecificType(), newGuid).Succeeded());
      }
    }
  }
  else if (pProp->GetCategory() == nsPropertyCategory::Map)
  {
        ...
        !pProp->GetFlags().IsSet(nsPropertyFlags::PointerOwner))
    {
      ...
      nsVariant value2 = GetVariantFromType(pProp->GetSpecificType()->GetVariantType());
      NS_TEST_BOOL(pObjectAccessor->InsertValue(pObject, pProp, value1, "value1").Succeeded());
      NS_TEST_BOOL(pObjectAccessor->InsertValue(pObject, pProp, value2, "value2").Succeeded());
    }
    else if (pProp->GetFlags().IsSet(nsPropertyFlags::Class))
    {
      nsInt32 iCurrentCount = pObject->GetTypeAccessor().GetCount(pProp->GetPropertyName());
      ...
      nsUuid newGuid = nsUuid::MakeUuid();
      NS_TEST_BOOL(pObjectAccessor->AddObject(pObject, pProp, "value1", pProp->GetSpecificType(), newGuid).Succeeded());
    }
  }
}
