...
class NS_UTILITIES_DLL nsDynamicOctree
{
...
public:
  ...
  void CreateTree(const nsVec3& vCenter, const nsVec3& vHalfExtents, float fMinNodeSize); // [tested]
  ...
  nsUInt32 GetCount() const { return m_NodeMap.GetCount(); } // [tested]
  ...
  ///
  /// Returns NS_SUCCESS when an object is inserted, NS_FAILURE when the object was rejected. The latter can only happen when bOnlyIfInside
  ...
  nsResult InsertObject(const nsVec3& vCenter, const nsVec3& vHalfExtents, nsInt32 iObjectType, nsInt32 iObjectInstance,
    nsDynamicTreeObject* out_pObject = nullptr, bool bOnlyIfInside = false); // [tested]
  ...
  /// purposes.
  void FindVisibleObjects(const nsFrustum& viewfrustum, NS_VISIBLE_OBJ_CALLBACK callback, void* pPassThrough) const;
  ...
  void FindObjectsInRange(const nsVec3& vPoint, NS_VISIBLE_OBJ_CALLBACK callback, void* pPassThrough = nullptr) const; // [tested]
  ...
  void FindObjectsInRange(const nsVec3& vPoint, float fRadius, NS_VISIBLE_OBJ_CALLBACK callback,
    void* pPassThrough = nullptr) const; // [tested]

  /// \brief Removes the given Object. Attention: This is an O(n) operation.
  void RemoveObject(nsInt32 iObjectType, nsInt32 iObjectInstance); // [tested]

  /// \brief Removes the given Object. This is an O(1) operation.
  void RemoveObject(nsDynamicTreeObject obj); // [tested]

  /// \brief Removes all Objects of the given Type. This is an O(n) operation.
  void RemoveObjectsOfType(nsInt32 iObjectType); // [tested]
  ...
  const nsBoundingBox& GetBoundingBox() const { return m_BBox; } // [tested]

private:
  /// \brief Recursively checks in which node an object is located and stores it at the node where it fits best.
  bool InsertObject(const nsVec3& vCenter, const nsVec3& vHalfExtents, const nsDynamicTree::nsObjectData& Obj, float minx, float maxx, float miny,
    float maxy, float minz, float maxz, nsUInt32 uiNodeID, nsUInt32 uiAddID, nsUInt32 uiSubAddID, nsDynamicTreeObject* out_Object);

  /// \brief Recursively checks which nodes are visible and calls the callback for each object at those nodes.
  void FindVisibleObjects(const nsFrustum& Viewfrustum, NS_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float miny,
    float maxy, float minz, float maxz, nsUInt32 uiNodeID, nsUInt32 uiAddID, nsUInt32 uiSubAddID, nsUInt32 uiNextNodeID) const;

  /// \brief Recursively checks in which node a point is located and calls the callback for all objects at those nodes.
  bool FindObjectsInRange(const nsVec3& vPoint, NS_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float miny, float maxy,
  ...
  bool FindObjectsInRange(const nsVec3& vPoint, float fRadius, NS_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx,
