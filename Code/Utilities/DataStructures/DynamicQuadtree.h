...
class NS_UTILITIES_DLL nsDynamicQuadtree
{
...
public:
  ...
  void FindVisibleObjects(const nsFrustum& viewfrustum, NS_VISIBLE_OBJ_CALLBACK callback, void* pPassThrough = nullptr) const;
...
private:
  ...
  bool InsertObject(const nsVec3& vCenter, const nsVec3& vHalfExtents, const nsDynamicTree::nsObjectData& Obj, float minx, float maxx, float minz,
    float maxz, nsUInt32 uiNodeID, nsUInt32 uiAddID, nsUInt32 uiSubAddID, nsDynamicTreeObject* out_Object);

  /// \brief Recursively checks which nodes are visible and calls the callback for each object at those nodes.
  void FindVisibleObjects(const nsFrustum& Viewfrustum, NS_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float minz,
    float maxz, nsUInt32 uiNodeID, nsUInt32 uiAddID, nsUInt32 uiSubAddID, nsUInt32 uiNextNodeID) const;

  /// \brief Recursively checks in which node a point is located and calls the callback for all objects at those nodes.
  bool FindObjectsInRange(const nsVec3& vPoint, NS_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx, float minz, float maxz,
    nsUInt32 uiNodeID, nsUInt32 uiAddID, nsUInt32 uiSubAddID, nsUInt32 uiNextNodeID) const;
