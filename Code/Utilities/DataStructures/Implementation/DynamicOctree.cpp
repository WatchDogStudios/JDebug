...
const float nsDynamicOctree::s_fLooseOctreeFactor = 1.1f;

nsDynamicOctree::nsDynamicOctree() = default;

void nsDynamicOctree::CreateTree(const nsVec3& vCenter, const nsVec3& vHalfExtents, float fMinNodeSize)
{
  ...
  float fMax = nsMath::Max(vHalfExtents.x, nsMath::Max(vHalfExtents.y, vHalfExtents.z));

  m_BBox = nsBoundingBox::MakeFromCenterAndHalfExtents(vCenter, nsVec3(fMax));
  ...
  for (nsUInt32 i = 0; i < m_uiMaxTreeDepth; ++i)
    m_uiAddIDTopLevel += nsMath::Pow(8, i);
}
...
nsResult nsDynamicOctree::InsertObject(const nsVec3& vCenter, const nsVec3& vHalfExtents, nsInt32 iObjectType, nsInt32 iObjectInstance,
  nsDynamicTreeObject* out_pObject, bool bOnlyIfInside)
{
  ...
  if (!InsertObject(vCenter, vHalfExtents, oData, m_BBox.m_vMin.x, m_BBox.m_vMax.x, m_BBox.m_vMin.y, m_BBox.m_vMax.y, m_BBox.m_vMin.z,
        m_BBox.m_vMax.z, 0, m_uiAddIDTopLevel, nsMath::Pow(8, m_uiMaxTreeDepth - 1), out_pObject))
  {
    if (!bOnlyIfInside)
    {
...
bool nsDynamicOctree::InsertObject(const nsVec3& vCenter, const nsVec3& vHalfExtents, const nsDynamicTree::nsObjectData& Obj, float minx, float maxx,
  float miny, float maxy, float minz, float maxz, nsUInt32 uiNodeID, nsUInt32 uiAddID, nsUInt32 uiSubAddID, nsDynamicTreeObject* out_Object)
{
  ...
  if (uiAddID > 0)
  {
    ...
    const nsUInt32 uiSubAddIDChild = uiSubAddID >> 3;
...
void nsDynamicOctree::FindObjectsInRange(const nsVec3& vPoint, NS_VISIBLE_OBJ_CALLBACK callback, void* pPassThrough) const
{
  ...
  FindObjectsInRange(vPoint, callback, pPassThrough, m_BBox.m_vMin.x, m_BBox.m_vMax.x, m_BBox.m_vMin.y, m_BBox.m_vMax.y, m_BBox.m_vMin.z,
    m_BBox.m_vMax.z, 0, m_uiAddIDTopLevel, nsMath::Pow(8, m_uiMaxTreeDepth - 1), 0xFFFFFFFF);
}

void nsDynamicOctree::FindVisibleObjects(const nsFrustum& viewfrustum, NS_VISIBLE_OBJ_CALLBACK callback, void* pPassThrough) const
{
  NS_ASSERT_DEV(m_uiMaxTreeDepth > 0, "nsDynamicOctree::FindVisibleObjects: You have to first create the tree.");
  ...
  FindVisibleObjects(viewfrustum, callback, pPassThrough, m_BBox.m_vMin.x, m_BBox.m_vMax.x, m_BBox.m_vMin.y, m_BBox.m_vMax.y, m_BBox.m_vMin.z,
    m_BBox.m_vMax.z, 0, m_uiAddIDTopLevel, nsMath::Pow(4, m_uiMaxTreeDepth - 1), 0xFFFFFFFF);
}

void nsDynamicOctree::FindVisibleObjects(const nsFrustum& Viewfrustum, NS_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx,
  float miny, float maxy, float minz, float maxz, nsUInt32 uiNodeID, nsUInt32 uiAddID, nsUInt32 uiSubAddID, nsUInt32 uiNextNodeID) const
{
  nsVec3 v[8];
  ...
  nsVolumePosition::Enum pos = Viewfrustum.GetObjectPosition(&v[0], 8);
...
void nsDynamicOctree::RemoveObject(nsDynamicTreeObject obj)
{
  m_NodeMap.Remove(obj);
}

void nsDynamicOctree::RemoveObject(nsInt32 iObjectType, nsInt32 iObjectInstance)
{
...
void nsDynamicOctree::RemoveObjectsOfType(nsInt32 iObjectType)
{
...
bool nsDynamicOctree::FindObjectsInRange(const nsVec3& vPoint, NS_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx,
  float miny, float maxy, float minz, float maxz, nsUInt32 uiNodeID, nsUInt32 uiAddID, nsUInt32 uiSubAddID, nsUInt32 uiNextNodeID) const
{
  if (vPoint.x < minx)
    return true;
...
void nsDynamicOctree::FindObjectsInRange(const nsVec3& vPoint, float fRadius, NS_VISIBLE_OBJ_CALLBACK callback, void* pPassThrough) const
{
  NS_ASSERT_DEV(m_uiMaxTreeDepth > 0, "nsDynamicOctree::FindObjectsInRange: You have to first create the tree.");
...
bool nsDynamicOctree::FindObjectsInRange(const nsVec3& vPoint, float fRadius, NS_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx,
  float maxx, float miny, float maxy, float minz, float maxz, nsUInt32 uiNodeID, nsUInt32 uiAddID, nsUInt32 uiSubAddID, nsUInt32 uiNextNodeID) const
{
