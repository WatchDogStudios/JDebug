...
const float nsDynamicQuadtree::s_fLooseOctreeFactor = 1.1f;

nsDynamicQuadtree::nsDynamicQuadtree() = default;

void nsDynamicQuadtree::CreateTree(const nsVec3& vCenter, const nsVec3& vHalfExtents, float fMinNodeSize)
{
  ...
  float fMax = nsMath::Max(vHalfExtents.x, vHalfExtents.z);

  m_BBox = nsBoundingBox::MakeInvalid();
  ...
  m_uiAddIDTopLevel = 0;
  for (nsUInt32 i = 0; i < m_uiMaxTreeDepth; ++i)
    m_uiAddIDTopLevel += nsMath::Pow(4, i);
}
...
nsResult nsDynamicQuadtree::InsertObject(const nsVec3& vCenter, const nsVec3& vHalfExtents, nsInt32 iObjectType, nsInt32 iObjectInstance,
  nsDynamicTreeObject* out_pObject, bool bOnlyIfInside)
{
  NS_ASSERT_DEV(m_uiMaxTreeDepth > 0, "nsDynamicQuadtree::InsertObject: You have to first create the tree.");

  if (out_pObject)
    *out_pObject = nsDynamicTreeObject();
  ...
  m_BBox.m_vMin.y = nsMath::Min(m_BBox.m_vMin.y, vCenter.y - vHalfExtents.y);
  m_BBox.m_vMax.y = nsMath::Max(m_BBox.m_vMax.y, vCenter.y + vHalfExtents.y);


  nsDynamicTree::nsObjectData oData;
  ...
  if (!InsertObject(vCenter, vHalfExtents, oData, m_BBox.m_vMin.x, m_BBox.m_vMax.x, m_BBox.m_vMin.z, m_BBox.m_vMax.z, 0, m_uiAddIDTopLevel,
        nsMath::Pow(4, m_uiMaxTreeDepth - 1), out_pObject))
  {
    if (!bOnlyIfInside)
    {
      nsDynamicTree::nsMultiMapKey mmk;
      mmk.m_uiKey = 0;
...
void nsDynamicQuadtree::RemoveObject(nsDynamicTreeObject obj)
{
  m_NodeMap.Remove(obj);
}

void nsDynamicQuadtree::RemoveObject(nsInt32 iObjectType, nsInt32 iObjectInstance)
{
  for (nsDynamicTreeObject it = m_NodeMap.GetIterator(); it.IsValid(); ++it)
  {
...
void nsDynamicQuadtree::RemoveObjectsOfType(nsInt32 iObjectType)
{
  for (nsDynamicTreeObject it = m_NodeMap.GetIterator(); it.IsValid();)
  {
    if (it.Value().m_iObjectType == iObjectType)
    {
      nsDynamicTreeObject itold = it;
      ++it;
...
bool nsDynamicQuadtree::InsertObject(const nsVec3& vCenter, const nsVec3& vHalfExtents, const nsDynamicTree::nsObjectData& Obj, float minx,
  float maxx, float minz, float maxz, nsUInt32 uiNodeID, nsUInt32 uiAddID, nsUInt32 uiSubAddID, nsDynamicTreeObject* out_Object)
{
  ...
  if (uiAddID > 0)
  {
    ...
    const nsUInt32 uiNodeIDBase = uiNodeID + 1;
    const nsUInt32 uiAddIDChild = uiAddID - uiSubAddID;
    const nsUInt32 uiSubAddIDChild = uiSubAddID / 4;
...
void nsDynamicQuadtree::FindVisibleObjects(const nsFrustum& viewfrustum, NS_VISIBLE_OBJ_CALLBACK callback, void* pPassThrough) const
{
  NS_ASSERT_DEV(m_uiMaxTreeDepth > 0, "nsDynamicQuadtree::FindVisibleObjects: You have to first create the tree.");
  ...
  FindVisibleObjects(viewfrustum, callback, pPassThrough, m_BBox.m_vMin.x, m_BBox.m_vMax.x, m_BBox.m_vMin.z, m_BBox.m_vMax.z, 0, m_uiAddIDTopLevel,
    nsMath::Pow(4, m_uiMaxTreeDepth - 1), 0xFFFFFFFF);
}

void nsDynamicQuadtree::FindVisibleObjects(const nsFrustum& Viewfrustum, NS_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx,
  float minz, float maxz, nsUInt32 uiNodeID, nsUInt32 uiAddID, nsUInt32 uiSubAddID, nsUInt32 uiNextNodeID) const
{
  ...
  const nsVolumePosition::Enum pos = Viewfrustum.GetObjectPosition(&v[0], 8);

  // stop traversal, if node is outside view-frustum
  if (pos == nsVolumePosition::Outside)
    return;
  ...
  nsDynamicTreeObjectConst it1 = m_NodeMap.LowerBound(mmk);
  ...
  if (pos == nsVolumePosition::Inside)
  {
    ...
    while (it1.IsValid())
    {
      ...
      nsDynamicTreeObjectConst temp = it1;
      ++it1;
  ...
  else if (pos == nsVolumePosition::Intersecting)
  {
    ...
    if (uiAddID > 0)
    {
      ...
      const nsUInt32 uiNodeIDBase = uiNodeID + 1;
      const nsUInt32 uiAddIDChild = uiAddID - uiSubAddID;
      const nsUInt32 uiSubAddIDChild = uiSubAddID >> 2;
...
void nsDynamicQuadtree::FindObjectsInRange(const nsVec3& vPoint, NS_VISIBLE_OBJ_CALLBACK callback, void* pPassThrough) const
{
  NS_ASSERT_DEV(m_uiMaxTreeDepth > 0, "nsDynamicQuadtree::FindObjectsInRange: You have to first create the tree.");
...
bool nsDynamicQuadtree::FindObjectsInRange(const nsVec3& vPoint, NS_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx, float maxx,
  float minz, float maxz, nsUInt32 uiNodeID, nsUInt32 uiAddID, nsUInt32 uiSubAddID, nsUInt32 uiNextNodeID) const
{
  if (vPoint.x < minx)
    return true;
...

  {
    ...
    {
      nsDynamicTree::nsMultiMapKey mmk2;
      mmk2.m_uiKey = uiNodeID + 1;

      const nsDynamicTreeObjectConst itlast = m_NodeMap.LowerBound(mmk2);
...
void nsDynamicQuadtree::FindObjectsInRange(const nsVec3& vPoint, float fRadius, NS_VISIBLE_OBJ_CALLBACK callback, void* pPassThrough) const
{
  NS_ASSERT_DEV(m_uiMaxTreeDepth > 0, "nsDynamicQuadtree::FindObjectsInRange: You have to first create the tree.");
  ...
  if (!m_BBox.Overlaps(nsBoundingBox::MakeFromMinMax(vPoint - nsVec3(fRadius), vPoint + nsVec3(fRadius))))
    return;
  ...
  FindObjectsInRange(vPoint, fRadius, callback, pPassThrough, m_BBox.m_vMin.x, m_BBox.m_vMax.x, m_BBox.m_vMin.z, m_BBox.m_vMax.z, 0,
    m_uiAddIDTopLevel, nsMath::Pow(4, m_uiMaxTreeDepth - 1), 0xFFFFFFFF);
}

bool nsDynamicQuadtree::FindObjectsInRange(const nsVec3& vPoint, float fRadius, NS_VISIBLE_OBJ_CALLBACK Callback, void* pPassThrough, float minx,
  float maxx, float minz, float maxz, nsUInt32 uiNodeID, nsUInt32 uiAddID, nsUInt32 uiSubAddID, nsUInt32 uiNextNodeID) const
{
