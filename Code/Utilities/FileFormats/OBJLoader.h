...
/// is preserved in the nsOBJLoader object. For upload into a GPU the vertex information must be duplicated manually.
class NS_UTILITIES_DLL nsOBJLoader
{
public:
  ...
  struct FaceVertex
  {
    ...
    nsUInt32 m_uiPositionID; ///< Index into the m_Positions array
    nsUInt32 m_uiNormalID;   ///< Index into the m_Normals array
    nsUInt32 m_uiTexCoordID; ///< Index into the m_TexCoords array
  };
  ...
  struct Material
  {
    ...
    nsString m_sDiffuseTexture;

    /// The ID of this material.
    nsUInt32 m_uiMaterialID;
  };
  ...
  struct Face
  {
    ...
    nsHybridArray<FaceVertex, 4> m_Vertices;

    /// Less-than operator is needed for sorting faces by material.
    NS_ALWAYS_INLINE bool operator<(const Face& rhs) const { return (m_uiMaterialID < rhs.m_uiMaterialID); }
  };
  ...
  ///
  /// Returns NS_FAILURE if the given file could not be found.
  nsResult LoadOBJ(const char* szFile, bool bIgnoreMaterials = false);
  ...
  ///
  /// Returns NS_FAILURE when the given file could not be found.
  nsResult LoadMTL(const char* szFile, const char* szMaterialBasePath = "");


  nsMap<nsString, Material> m_Materials;

  nsDeque<nsVec3> m_Positions;
  nsDeque<nsVec3> m_Normals;
  nsDeque<nsVec3> m_TexCoords;
  nsDeque<Face> m_Faces;
};
